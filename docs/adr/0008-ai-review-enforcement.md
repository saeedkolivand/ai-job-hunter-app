...
# Mandatory AI review enforcement via deterministic schema-1 verdicts across three active surfaces (Stop gate, /review, CI) — and historical pre-push lane
...
2. **Strong opt-in** — `@claude review` on-demand review (tag-mode `claude-review.yml`) is agency-aware, but it is never automatic and crucially **never blocks merge** — the owner can still commit/push without reviewing findings. (Inline-comment posting was a silent no-op at the time; see Amendments.)
...
- **Model tier:** Claude Sonnet 5 for the CI gate (lower latency/cost than Opus for a deterministic diff review that doesn't need agency chains). Opus 5 still used for the on-demand `/review` deep dive (agent routing).
...
- **Amendment (2026-08-01):** Inline-comment posting was non-functional in the original action (MCP server only started when `allowedTools` included `mcp__github_inline_comment__*`, which tag mode never supplied). This is fixed: Lane A1 (`claude-review` job) now posts inline comments for all findings; Lane A2 (`ai-review-gate` job) posts inline comments only for blocking findings (HIGH/CRITICAL, confidence ≥ 0.8, introduced_by_diff: true) — that lane re-runs on every `synchronize` and inline comments are **not** deduplicated across runs, so an unbounded rule would stack a fresh copy of every nit on each push. The threshold matches `scripts/ci-review-verdict.mjs` so inline comments and the red check never disagree; the sticky comment remains the full-finding-list surface.
