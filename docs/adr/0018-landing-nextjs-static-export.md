...
**No structural DOM gate exists.** `scripts/diff-dom.mjs` (element-by-element DOM comparison against pre-refactor baselines) was built for the conversion above and **has since been deleted**, along with its test — once `src/content/*/body.html` was removed it had no input to compare against, and `check-parity.mjs` never invoked it. This means **markup changes to the landing pages are not caught by an automated regression gate** — only text and link presence. The structural defence is code review plus, for any large markup move, a manual before/after diff of the built `out/*.html`: compare the tag sequence (element name + `id` + `class`, in document order) and the whitespace-collapsed visible text, stripping `<script>` blocks from both first. That check is what verified the component split in #917; strip the scripts, or Next's per-build RSC hydration payload and random `buildId` will show spurious drift.
