...
- **Marketing tier link**: One additive link in `src/components/home/sections/Finale.tsx` (the anchor element with `href="/world"` and text `→ or fly through the world (new)`, rendered in the Finale section). No other marketing copy touched; the marketing tier remains protected.
