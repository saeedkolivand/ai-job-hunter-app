...
- **Superseded (task #16 / [ADR 0012](0012-ai-provider-base-url-provenance.md)):** the enable-time provider/model snapshot described above is **retired**. `resolve_answer_assist` now reads the active provider live from the shared, backend-owned `AiConfigStore` — the same source `ai_generate` uses — instead of a per-toggle `AiAssistConfig` snapshot. `extension_bridge_set_ai_assist_enabled` is now a bare boolean; the "immutable until re-toggle" behavior no longer applies — an assist call always uses whatever provider is currently active. The **opt-in/consent decision itself** (a separate, billable-tier gate distinct from free autofill) is unchanged and still governed by this ADR.
