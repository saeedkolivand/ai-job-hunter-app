...
- **Max (Phase 4, shipped)** — quality + section-wise JSON generation over a seeded roster (8-company cap + condensed earlier-roles). Sequential per-section calls via `Completer::complete_json` feeding an assembled document. Deterministic validate + Warning-only judge (runs last, skippable) + repair. Strategy and evidence artifacts persist (full clamp for oversized, truncation fails hard) enabling per-entry regenerate via persisted CompanyPlan + citations. Absence-aware repair accept: a round is worse if the draft carries an absence-shaped Critical (dropped_role / altered_project_link absence) the prior round did not. Source-relative save gate: a draft losing the source's employment section is refused (unless the source has none). Section timeline UI tracks per-section states (queued→generating→done→checking→needsChanges/repaired/clean); progressive assembly via `ai:stream` appends per-section text; events carry `sectionKey` (summary | skills | experience:<i> | projects | education).
