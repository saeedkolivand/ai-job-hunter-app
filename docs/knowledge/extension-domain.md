...
| `answer.assist` / `assist.chunk` / `assist.done` / `answer.assist.result` | Extension → Desktop             | `answer_assist_enabled` ([ADR 0011](../adr/0011-extension-ai-assist-optin.md)) | Streaming frames: chunk `{ delta }`, done (no payload), result `{ ok, question, draft, sourced }`          | Billable AI drafting; provider/model snapshotted at enable time; per-connection per-reqId registration via `AssistStreamRegistry`; streaming chunks ordered per-reqId; can cancel before compose |
