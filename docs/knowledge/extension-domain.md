...
Last updated: 2026-07-16 (task #16: `answer.assist` routing moved from a per-toggle provider snapshot to the shared backend `AiConfigStore`)
...
| `answer.assist` / `assist.chunk` / `assist.done` / `assist.cancel` / `answer.assist.result` | Extension → Desktop             | AI-assist opt-in via `BridgeState` ([ADR 0011](../adr/0011-extension-ai-assist-optin.md)) | Request/streaming: assist `{ question, url?, search_web? }`, chunk `{ delta }`, cancel (no payload), done (no payload), result `{ ok, question, draft, sourced }` | Billable AI drafting; gate is now a **bare boolean** (`ai_assist_enabled: AtomicBool`) — no provider/model/base_url snapshot. Routing resolves live from the backend-owned `AiConfigStore` via `Completer::from_active` ([ADR 0012](../adr/0012-ai-provider-base-url-provenance.md)), same source as `ai_generate`; per-connection per-reqId registration via `AssistStreamRegistry`; streaming chunks ordered per-reqId; `assist.cancel` marks for early-abort before compose starts |
