...
**12. One reader, one dispatcher, one writer; bridge-backed calls single-flight in input order**: Three threads over one `Event` channel: a READER thread turns stdin into `Event::Line`s and `Event::Eof`; one WORKER thread owns the tokio runtime and runs bridge-backed tool calls (each under `INVOCATION_TIMEOUT`), sending `Event::Reply` per call; the MAIN thread (the only writer) consumes events, classifies and answers local tools, protocol methods and refusals immediately from the writer thread, and writes replies in order per kind (bridge replies first, then immediate answers). Bridge-backed `tools/call`s queue FIFO and dispatch single-flight (exactly one in flight at any instant), so the throttle bound remains one bridge connection per process. Local tools (`commands`), unknown tools, parse errors and every local refusal answer immediately on the writer thread — no wire wait, no hung-server appearance on `ping`. Replies carry the `id` they answer, so JSON-RPC clients pair them regardless of order. EOF drains queued calls before exit 0. An `emit` error (EPIPE: client closed its pipe) exits immediately, 0.
