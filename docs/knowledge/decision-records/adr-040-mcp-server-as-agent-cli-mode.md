...
**12. One reader, one dispatcher, one writer; bridge-backed calls single-flight in input order**: Three threads over one `Event` channel: a READER thread turns stdin into `Event::Line`s and `Event::Eof`; one WORKER thread owns the tokio runtime and runs bridge-backed tool calls (each under `INVOCATION_TIMEOUT`), sending `Event::Reply` per call; the MAIN thread (the only writer) consumes events, classifies and answers local tools, protocol methods and refusals immediately from the writer thread, and writes replies in order per kind, never globally (an immediate answer may be written before the reply of a bridge call that arrived earlier; two bridge replies are never reordered against each other). Bridge-backed `tools/call`s queue FIFO and dispatch single-flight (exactly one in flight at any instant), so the throttle bound remains one bridge connection per process. Local tools (`commands`), unknown tools, parse errors and every local refusal answer immediately on the writer thread — no wire wait, no hung-server appearance on `ping`. Replies carry the `id` they answer, so JSON-RPC clients pair them regardless of order. The queue of bridge-backed calls is bounded (the constant lives beside the serve loop); a call arriving while it is full is refused with its own sentinel rather than buffered, and the reader-to-writer event queue is bounded too, with the reader blocking on it — so a client cannot grow the process without limit: the pipe's lost backpressure is replaced by an explicit refusal on the dispatch side and by a blocking reader on the input side. On EOF the loop drains queued calls under one absolute deadline (the per-invocation timeout); when it expires, every call still owed a reply is answered with a `shutting_down` result whose `dispatched` flag says whether it was already in flight (and may have taken effect) or never reached the app (safe to resend to a new server), and the process exits 0. Nothing handed to the worker is left unanswered, and shutdown is bounded rather than N times the timeout. An `emit` error (EPIPE: client closed its pipe) exits immediately, 0.
...
- **Keepalive pings are now answered mid-call**: the reader/writer split (§12) means `ping` and local tools are answered immediately while a bridge call is in flight, so a client keepalive no longer mistakes a slow dispatch for a hung server. Still unverified: real client keepalive behaviour during a long call.
...
- **Tool results carry PII to the client**: Results leave the machine and land in the MCP client's cloud provider and persisted transcript. The `profile` tool returns contact info; `documents_get_text` returns the full résumé; `applications_list` and similar return application records. The server carries no redaction (per ADR-038 §3, the generic tier is raw). This is an explicit consequence of MCP design, not a new risk.
  ...
  [ADR-038](adr-038-agent-cli-full-parity-two-tier.md)'s 2026-09-07 amendment widened it rather than narrowing it. The whole-table list commands now page instead of being refused whole, and a document export now travels base64 instead of overrunning the cap — `PAGINATED_LIST_COMMANDS` and `BASE64_BYTE_FIELDS` in `agent_call/reshape.rs` name the audited rows. So the generated résumé and cover-letter text, the scraped ads and application records they sit beside, and the exported PDF's bytes are now all reachable into the provider and the transcript — the lists a page at a time, the export in one base64 result — where the result cap previously refused them wholesale. Fencing does not narrow it either: it marks third-party posting text as untrusted, while the user's own `resume_text`/`cover_letter_text` and the export bytes are outside it by design (ADR-038 §3 and its ADR-0005 amendment). Trading a refusal for delivery was the point of the fix; this records the accepted risk at its real size.
...
- **Keepalive path covered by tests with a blocking dispatch stub**: the reader/writer split is exercised by a test that sends `ping` while a tool call is blocked on a dispatch stub; the ping is answered immediately on the writer thread.
- **The rewritten loop, against the real binary over stdio (app not running):** `initialize` → `notifications/initialized` → `tools/call commands` → `ping` → `tools/list` produced one frame per request in input order (the local `commands` tool answered inline, ahead of the queue), zero bytes on stderr, exit 0; and `initialize` → `tools/call profile` with stdin closed immediately still wrote the bridge-backed reply (the `app_not_running` refusal) before exiting 0, which is the EOF drain in practice. The bounded queues and the `shutting_down` deadline path are covered by unit tests with injected budgets, not by a live client.
- **Against the real clients, after merge (2026-09-02), captured through a stdio tee shim:** Claude Code 2.1.258, registered with `claude mcp add`, health-checks with `initialize` (`protocolVersion` 2025-11-25; capabilities `roots`, `elicitation`) → `notifications/initialized` → `tools/list`, and a `claude -p` turn issued a real `tools/call` for `profile` — the request carries `_meta.claudecode/toolUseId` and a `progressToken`, both correctly ignored — and the model answered from the result. Codex 0.144.6, registered with `codex mcp add`, sends `initialize` with `protocolVersion` 2025-06-18 (`clientInfo.name` `codex-mcp-client`; elicitation `form` + `url`) and a real `tools/call` carrying `_meta["x-codex-turn-metadata"]` (session, thread and turn ids, sandbox), answered from the running app. Neither client pinged mid-call in these runs.
- **Irreversible confirm path**: the pure core of the matching proof logic is tested with match/mismatch/unavailable cases; a live dispatch with a matching proof is still unverified.
- **Both recorded sessions are now a test, on every platform CI runs** (2026-09-03): the two scripted stdio sessions above live in `apps/desktop/src-tauri/tests/mcp_smoke.rs`, driving the real linked binary over real pipes. They GATE on Linux, where they ride the ordinary tests job as an ordinary integration test, and run advisory on macOS and Windows in a matrix of their own (`.github/workflows/quality.yml`) — which closes the macOS/Linux smoke gap this list used to name. A debug binary is console-subsystem, so this still does not prove the release Windows GUI-subsystem stdout path.
- **Correction to the second session's sentinel**: on a machine that never launched the app the refusal is `app_not_located`, not the `app_not_running` recorded above. A bridge-backed tool reads the agent-CLI pointer file under the user's home before it tries a loopback connect, so the earlier value was an artefact of recording the session on a dev box that had one. The test pins a fresh empty home per session, which makes the outcome identical on a CI runner and on a developer's machine with the app open.
- **The path is now in the UI**: a Settings → Developer card surfaces the binary path and the ready-to-copy Claude Code and Codex registration snippets, fed by `commands::system::system_agent_cli_info`, a renderer-only command classified `NotExposed` in the policy table because an agent client already holds the path it launched the binary from — the same resolver that writes the pointer file's `exePath`, so the card and the file cannot disagree.
- **Platform coverage**: the modern wire remains unverified.
