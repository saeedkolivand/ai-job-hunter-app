...
- **The path is now in the UI**: a Settings → Developer card surfaces the binary path and the ready-to-copy Claude Code and Codex registration snippets, fed by the `Effect::Read` command `commands::system::system_agent_cli_info` — the same resolver that writes the pointer file's `exePath`, so the card and the file cannot disagree.
