...
- **Tool results carry PII to the client**: Results leave the machine and land in the MCP client's cloud provider and persisted transcript. The `profile` tool returns contact info; `documents_get_text` returns the full résumé; `applications_list` and similar return application records. The server carries no redaction (per ADR-038 §3, the generic tier is raw). This is an explicit consequence of MCP design, not a new risk.
  ...
  [ADR-038](adr-038-agent-cli-full-parity-two-tier.md)'s 2026-09-07 amendment widened it rather than narrowing it. The whole-table list commands now page instead of being refused whole, and a document export now travels base64 instead of overrunning the cap — `PAGINATED_LIST_COMMANDS` and `BASE64_BYTE_FIELDS` in `agent_call.rs` name the audited rows. So the generated résumé and cover-letter text, the scraped ads and application records they sit beside, and the exported PDF's bytes are now all reachable into the provider and the transcript — the lists a page at a time, the export in one base64 result — where the result cap previously refused them wholesale. Fencing does not narrow it either: it marks third-party posting text as untrusted, while the user's own `resume_text`/`cover_letter_text` and the export bytes are outside it by design (ADR-038 §3 and its ADR-0005 amendment). Trading a refusal for delivery was the point of the fix; this records the accepted risk at its real size.
