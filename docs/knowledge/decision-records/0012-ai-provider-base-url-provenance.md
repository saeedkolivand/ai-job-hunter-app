...
- Resolved follow-up: `agent_run` was flipped off the request-supplied `base_url` (see the removed `Completer::resolve` constructor in `pipeline/mod.rs`).
