...
**Auto-write wired status:** shipped behaviour is v2 auto-write, gated in order by the opt-in toggle (default OFF), then sender provenance, then a decided intent; any of the three failing is a silent no-op. Which module owns the call, and why the layer rules put it there rather than in the poller, is enforced by `tests/architecture.rs` and documented on `email_watch::auto_write::apply_matched_intent` — not restated here, because a layering decision that drifts in prose is worse than one nobody copied.
...
**Sender provenance gate:** an auto-write requires a recognised sender domain on the **visible `From:` header** — not the SMTP envelope, which the sender controls freely — together with an aligned DMARC pass read from the topmost `Authentication-Results` header. A cold email from an unrecognised sender never writes, even if it fingerprints and classifies.

The list that authorises a write is deliberately **narrower** than the one that boosts a match score, and drops the large relays that carry third-party text from their own valid infrastructure. Keeping them separate is the decision: a heuristic tuned for recall must never double as an authorisation, and this ADR previously named the scoring list here, which quietly re-merged the two roles. The lists and the predicate that reads them live in `email_watch::parser`; treat that as the source of truth rather than copying either list into prose again.

Two wider signals were considered and deferred: matching the sender domain against the company's expected domain, and thread linkage via `References`/`In-Reply-To`.
...
- **A backup restore settles what was still provisional, and forgets what was rejected.** `ApplicationStore::import` deletes every `status_events` row and re-seeds one `source = 'user', confirmed = true` "imported" event per application. So a status that was an unadjudicated email-derived write at export time returns as settled user history — no provisional badge, no Accept/Reject, and no record that an email asserted it. Every `email_reject` row is deleted with the rest, which is the memory `was_transition_rejected` reads, so a target the user explicitly disputed can be re-applied by a later email. Accepted rather than fixed: a restore is a deliberate replace-everything action, and any re-applied write still lands unconfirmed and adjudicable. Closing it properly means exporting and importing email-sourced `status_events` rows, which is its own change.
...
- **New IPC commands** (5-step): `email_watch_status()`, `email_watch_connect(address, appPassword)`, `email_watch_disconnect()`, `email_watch_set_enabled(bool)`, `email_watch_check_now()`, and `email_watch_set_auto_write_enabled(bool)` (v2). Commands are backend-owned; the renderer never supplies a host/port (per ADR-0012).
- **New Settings UI** section "Email tracking" in Accounts — email and app-password inputs, Connect/Disconnect, enabled toggle, status line (address, last check, "Check now" button), the v2 auto-write toggle (default OFF, opt-in), and a consent disclosure (full mailbox access, keychain storage, zero egress, and what auto-write does once the user enables it).
...
- IPC: `apps/desktop/src-tauri/src/commands/email_watch.rs` (6 commands).
