...
8. **Amendment (2026-09-16, PR4 — Prep tab + save-answers-on-submit) — a second, distinct auto-write consent class, plus a zero-spend read surface.** Two things land together here, kept separate on purpose. The **Prep tab** is a pure read of the user's own already-generated per-job data (company brief, interview questions, salary answer) through the extension read tier, at zero additional AI spend — nothing new to gate beyond the read tier's existing Autofill opt-in — with two on-demand drafts (company brief, salary answer) that ride the EXISTING billable `answer.assist` verb under the separate AI-assist opt-in ([ADR 0011](0011-extension-ai-assist-optin.md)), never a new verb or a new spend bucket. **Saving the answers the user typed at submit time is its OWN consent class** — a further amendment to decision 7's auto-track shape, not folded into it and not folded into decision 5's autofill opt-in — because the trigger here is a detected event (a form submit) rather than a click on the feature itself, exactly like decision 7's submit-watch trigger, AND because what it writes is page-derived answer TEXT rather than flipping one status enum value: a wrong auto-track flip is reversible from the stage picker in one click, while a captured answer is free text that could carry anything the user typed into that field at that instant. A single toggle covering both would let turning on "mark applied automatically" silently start writing form text too — collapsing two risk profiles the user never asked to accept together.
   - **The enforced boundary is server-side, not client-side**, mirroring decision 7's split exactly: `answers_save::auto_save_refused` (`apps/desktop/src-tauri/src/extension_bridge/answers_save.rs`) refuses any `auto:true` `answers.save` write unless the desktop's own `BridgeState::save_answers_on_submit_enabled()` is true — checked against desktop-owned state, never against anything the extension asserts about itself. The extension-side arming check (whether `submit-watch` even captures) is **defense-in-depth only**.
   - **Honest residual risk (disclosed, not hidden):** an answer captured at submit time is whatever text was sitting in that field at that instant — there is no human review step between typing and the auto-save, unlike the deliberate "Save my answers" click this sits beside. It lands on an existing tracked Application only (the same `saved`/`applied` job decision 7 already tracks), never creates one, and stays bounded by the same per-question/per-answer/per-call byte caps every `answers.save` call already enforces.
   - New wire surface: the optional `auto` flag on `answers.save` (mirrors `status.update`'s), the fourth `SettingsKey::SaveAnswersOnSubmit` on `settings.get`/`settings.set`, and the read-only `prep` resource on the extension read tier (`extension_bridge/agent_read/prep.rs`) — no new bridge verb pair for the Prep tab itself.
...
- **(PR4) A captured-answer auto-write is its own consent class, distinct from Task #22's status auto-write** — same desktop-side enforcement discipline (`answers_save::auto_save_refused` mirrors `status_update::auto_write_refused`), but never shares a toggle with auto-track: flipping "mark applied automatically" must never silently start writing page text too. The Prep tab's reads cost nothing new — no new gate, no new spend bucket — because it is the user's own already-generated data crossing the same read tier decision 1 already opened.
...
- **Prep tab + save-answers-on-submit (PR4 amendment):** read resource — `extension_bridge/agent_read/prep.rs` (`prep_resource`/`project_generation`, caps + `truncated` flag); on-demand drafts — `extension_bridge/answer_assist_topic.rs` (`topic` field on `answer.assist`, draft-mode-only); server-side enforcement — `extension_bridge/answers_save.rs` (`auto_save_refused`/`is_auto_answers_save`); opt-in state — `extension_bridge/save_answers_optin.rs` (`BridgeState::save_answers_on_submit_enabled`/`set_save_answers_on_submit_enabled`); settings wiring — `extension_bridge/settings.rs` (`SettingsKey::SaveAnswersOnSubmit`); deep link — `deeplink/mod.rs` (`FocusTarget::PrepForJob`, `ajh://prep?url=`), `tray/mod.rs` (`dispatch_prep_for_job`); extension side — `apps/extension/src/prep/prep.ts` (the Prep tab), `apps/extension/src/lib/auto-save-notice.ts` (the transparent post-save notice), `apps/extension/src/lib/submit-watch.ts` (synchronous capture), `apps/extension/src/lib/auto-track.ts` (the nested toggle).
