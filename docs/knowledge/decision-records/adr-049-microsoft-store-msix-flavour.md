...
- **Data and registrations are shared across flavours.** A user can switch between the packaged store and the bare installer (or between Snap and Flatpak on Linux) without a migration — the app data directory and HKCU registrations (Windows) stay in the same real locations.
...
- **(Snap/Flatpak) Native messaging is unavailable.** A browser cannot reliably reach the host process inside a strict sandbox, even via breakaway mechanisms — Snap's `flatpak-spawn --host` breaks isolation by design; Flatpak has no equivalent. The extension bridge works for in-sandbox scenarios (e.g., GNOME Web). This is a disclosed limitation, listed in `docs/DEPLOYMENT.md`.
