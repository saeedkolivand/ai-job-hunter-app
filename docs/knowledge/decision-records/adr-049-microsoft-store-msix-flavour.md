...
**Ship the Microsoft Store build as an MSIX that wraps the very same `ajh-tauri.exe` the NSIS installer ships** — a container, not a build variant, with no compile-time flag and no second binary. Which container is running is decided **at runtime** by `platform::msix::is_packaged()`, from package identity **plus** the running exe living inside the package's install root (identity alone is inherited by child processes, so it cannot tell the two apart on its own; both sides of that comparison are canonicalized first, since neither an 8.3 short name nor Windows' case rules survive a string compare). Missing or unreadable evidence counts as **packaged**, because the dangerous direction is a packaged build concluding it is unpackaged and re-enabling the GitHub updater.

Each behaviour that follows is owned by its own symbol rather than by this record:

- **Updating is the Store's.** The packaged build never checks and never polls; the additive marker it answers the updater contract with — one older clients still read as "no update" — and the refusals behind `download`/`install` live in `apps/desktop/src-tauri/src/updater/mod.rs` and `packages/shared/src/ipc/contracts/updater.ts`.
- **What it publishes about itself** is the execution-alias shim, never `current_exe()`. When no usable shim exists (a user can switch an execution alias off), it publishes **nothing**: the native-messaging registration and the agent-CLI pointer are skipped rather than written with a path the browser cannot launch — and rather than deleted, since they may belong to a non-Store install on the same machine. `platform::msix::published_exe_path`.
- **Launch at login** is the manifest's `StartupTask`, driven through awaited WinRT calls: it can be refused by the user or by policy, and that refusal has to reach the UI as an error instead of a silent "off" — while the wait for it must not block a shared Tokio worker.
- **Write virtualization is disabled** — which is what the restricted capability the manifest declares buys — so the HKCU native-messaging registration and the app data directory stay the same real, shared locations a non-Store install uses, and a user can move between flavours without losing data. The corollary is that the packaged build must not re-register what the manifest already declares.

Identity values are assigned by Partner Center and therefore supplied as GitHub repository **variables**, never committed, and the unsigned `.msix` is a workflow **artifact** only — never a GitHub Release asset. Submission is automated by the `publish-msstore` job via the `msstore` CLI, authenticating through an Entra app registration (Manager role) linked to the Partner Center account. The set of behaviours that differ is closed, and enumerated at `platform::msix` itself with `docs/DEPLOYMENT.md` § Microsoft Store (MSIX) as the operational view of it.
...
- **Every path a packaged build publishes about itself is a decision.** `current_exe()` is the wrong answer there; the execution-alias shim is the stable one, and "publish nothing, skip the write" is a real third answer that callers have to handle. Any future feature that records this app's own path must ask which it needs.
...
- `docs/DEPLOYMENT.md` § "Microsoft Store (MSIX)" — the operational page: identity variables, the local test loop, the manual verification checklist and the manual submission steps. This record is the _why_; that page is the _how_.
