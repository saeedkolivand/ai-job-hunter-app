...
Identity values are assigned by Partner Center and therefore supplied as GitHub repository **variables**, never committed, and the unsigned `.msix` is a workflow **artifact** only — never a GitHub Release asset. The first submission is manual; `msstore` automation waits for an Entra tenant to exist. The set of behaviours that differ is closed, and enumerated at `platform::msix` itself with `docs/DEPLOYMENT.md` § Microsoft Store (MSIX) as the operational view of it.
