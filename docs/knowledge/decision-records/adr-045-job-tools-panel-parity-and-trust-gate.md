...
- **A wrong-tab probe call is prevented for a tab this module has already evaluated** — `captureActiveTabFieldsProbe`/`activeTabUrl` are never invoked while `isPageTrusted` currently reads false. This is a caller contract, not a module-internal guarantee: `subscribeAnswerState`'s first delivery for a newly-followed tab is unavoidably asynchronous, so the module cannot itself know a caller just switched tabs until that delivery lands — `sidepanel.ts::follow` upholds its half by calling `checkPage()` only from inside that delivery, never as a separate statement right after subscribing (a real bug this exact shape produced once, caught in review, not by the first round of tests — see that function's doc).
