...
**3. Vectors are `f32`, not the existing `f64`.** The in-memory `PostingsCache` holds live `EmbeddingVector`s and is bounded by `DENSE_CANDIDATE_MAX` (see `apps/desktop/src-tauri/src/commands/hybrid_search.rs`); it is small enough that an approximate nearest-neighbor (ANN) index is unnecessary on scale grounds alone. `f32` halves memory and is sufficiently precise for this bounded ranking: rounding can perturb cosine values in the last few bits and so reorder near-tied postings, but no tie-tolerance guarantee is made or needed here — the rerank is the ordering authority when it runs, and the fused list is a candidate pool, not a scored ranking. Rejected: adding an ANN library (hnsw, usearch, sqlite-vec) — unnecessary complexity for a small, ephemeral cache.
