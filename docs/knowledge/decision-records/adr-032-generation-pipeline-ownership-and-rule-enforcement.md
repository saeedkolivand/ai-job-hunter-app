...
4. **Deterministic depth tiers:**
   ...
   - **Max** — quality with the single draft replaced by section-wise generation, then assemble → validate → repair → judge. The stage list is `pipeline::resume::MAX_STAGES` and the section ceiling is `Budget::RESUME_MAX.max_sections`; read them there rather than from a number copied here — this line said "≤9" while the budget said 12. The judge runs LAST, over the finished repaired document: its items are Warning-only by construction (`JudgeItem` has no severity field) and the stage is skippable, so no error, timeout or refused call can block a run. Strategy and evidence artifacts persist in full at max depth only (clamped; an unparseable one is a soft miss), which is what lets a per-entry regenerate rebuild one role from its stored `CompanyPlan` instead of re-running the pipeline.
