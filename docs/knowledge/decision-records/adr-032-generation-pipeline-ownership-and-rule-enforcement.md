...
2. **Three things are withdrawn from the model structurally:**
   - **Evidence grounding** — deterministic since #1269, with no provider call: Rust picks each requirement's best-supporting résumé bullet with the source keywords kernel and decides its `status` from the same kernel, guaranteeing the pipeline's coverage claim cannot disagree with the Jobs-page match score.
...
4. **The single staged pipeline** (`analyze_job` → `match_evidence` → `strategy` → `draft` → `cover_letter` → `validate` → `repair` → `humanize`) — one fixed list; which DOCUMENTS a run writes is a pair of request flags, not a second stage list, so `draft` and `cover_letter` each no-op in place rather than forking the vocabulary `paying_stages()` and `PIPELINE_STAGES` are derived from:
   - **Quality pipeline** — the ONLY production pipeline for the apply flow. Built across: #990 (deterministic projects normalization), #991 (cover-letter + humanize stages), #992 (apply-flow cutover to staged run). Base call count: 2 (analyze, strategy; `match_evidence` is free since #1269, see `PIPELINE_STAGES_FREE`) — the grounding stages every run pays for, because the letter prompt fences the strategy artifact too. Optional calls: draft (1 call, gated on `ResumePipelineRunSchema.includeResume`, which owns the `true` default, in `packages/shared/src/schemas/index.ts`), cover_letter (1 call, gated on `ResumePipelineRunSchema.includeCoverLetter`, which owns the `false` default, same file), repair (variable; only if Criticals exist; ≤2 rounds × `stages::repair::MAX_SECTIONS_PER_ROUND`, which owns that bound), humanize (0–2 calls; deterministic-first — the zero-flag early return in `stages::humanize::Humanize::run` owns the zero-cost path, and that same function owns the ≤1-call-per-document cap). Deterministic validation always runs; repair and humanize gate on report findings, never on assumptions. All stages are listed in `ipc_contracts::events::PIPELINE_STAGES` (generated from `packages/shared/src/events/pipeline.ts`).
