...
- **Language identity** (`documents::keywords::detected_language`) is a separate function from `languages_align` (stemming compatibility). It returns `Option<&'static str>` ISO-639-1 tag when `whatlang` reads the text reliably — the decision delegates to `whatlang`'s own `Info::is_reliable()`, which is _strictly_ greater than 0.9, so the bar cannot drift from the library's; `None` otherwise (covers 19 curated languages; every other language is a no-op, not a mis-detection). **Two detectors decide this question:** the renderer uses **franc**, Rust uses **whatlang**; when they disagree the guard goes quiet (as this module's documented posture demands).
