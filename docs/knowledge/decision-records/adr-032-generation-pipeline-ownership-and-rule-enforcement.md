...
4. **The single staged pipeline** (`analyze_job` → `match_evidence` → `strategy` → `draft` → `cover_letter` → `validate` → `repair` → `humanize`):
   - **Quality depth** — the ONLY production pipeline. Built across: #990 (deterministic projects normalization), #991 (cover-letter + humanize stages), #992 (apply-flow cutover to staged run). Base call count: 4 (analyze, evidence, strategy, draft). Optional calls: cover_letter (1 call, if `includeCoverLetter=true`; default false), repair (variable; only if Criticals exist; ≤2 rounds × `MAX_SECTIONS_PER_ROUND` sections), humanize (0–2 calls; deterministic-first: zero if no voice flags, ≤1 per document if flags exist post-filter). Deterministic validation always runs; repair and humanize gate on report findings, never on assumptions. All stages are listed in `ipc_contracts::events::PIPELINE_STAGES` (generated from `packages/shared/src/events/pipeline.ts`).
