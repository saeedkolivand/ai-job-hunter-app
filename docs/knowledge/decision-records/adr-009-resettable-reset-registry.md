...
- A `ResetRegistry` (managed Tauri state) holds type-erased reset closures. Stores are wired through `manage_resettable(app, &mut registry, label, store)` in `lib.rs::setup`, which **manages the store and registers its reset in one call** — so the `.manage()` site is the single place a store is wired for reset coverage.
