...
- **Verified against the source**: read as they stand today in `apps/extension/src/manifest.test.ts` — the both-targets parity assertion, the single allowlist literal, the README justification assertion that reads it, the denylist, and the no-content-scripts pin, of which the denylist and the no-content-scripts pin stay unchanged by this record. The assertions decision 7 moves are the both-targets equality, the per-target allowlist equality, and the README justification assertion that reads that allowlist. Also verified: that the TS↔Rust parity test in `apps/desktop/src-tauri/src/extension_bridge/test.rs` enumerates its constants by hand, which is why decision 8 has the delivering PR extend it; that the two refusal sentinels exist as fixed constants beside the handler in `apps/desktop/src-tauri/src/extension_bridge/answer_assist.rs`; that the rewrite mode and its existing-answer field already exist on the `answer.assist` request schema, so decision 5 needs no protocol change.
...
- **Not verified**: any behaviour in a built extension. This record is written ahead of the implementation, so the gesture chain in decision 2, the post-navigation state in decision 3, the older-desktop fallback in decision 6, and the two test amendments the delivering PR owes (the per-target permission pins in decision 7, the two sentinels added to the parity list in decision 8) remain claims that PR has to pin.
