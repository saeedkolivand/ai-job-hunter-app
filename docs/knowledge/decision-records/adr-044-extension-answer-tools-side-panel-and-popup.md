...
Decision 2 shipped one context-menu entry (`ajh-answer-selection`), scoped to `contexts: ['selection']`, so a plain right-click with nothing selected showed nothing from this extension. A second entry now ships alongside it: `ajh-answer-open-panel`, registered on `contexts: ['page', 'editable']`, titled to match the popup's own `#btn-open-panel` control (see `ANSWER_PANEL_MENU_ID`'s registration in `background.ts`), and its click handler calls `openAnswerPanel` directly with no row added — there is nothing to prefill.
