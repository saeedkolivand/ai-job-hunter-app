# ADR-044 — Extension Answer tools in a side panel and the popup: two views of one per-tab state, page access still user-gestured
...
**3. What the grant does after a navigation is stated in the UI.** Per Chrome's `activeTab` documentation (read 2026-09-03) the grant itself is per **tab** and lasts for the same-origin session: same-origin navigation keeps it, a cross-origin navigation revokes it. This extension cannot tell the two apart, though — reading the new URL to compare origins needs the `tabs` permission, which stays on the denylist (decision 7) — so `AnswerState.pageChanged` is set conservatively on EVERY navigation, same-origin included, and the write controls it gates are unavailable either way until the next scan. The underlying grant surviving a same-origin navigation buys nothing here; from the user's side, ANY navigation is a cross-origin one. After a navigation the panel **keeps its rows** — they are state the extension holds on its own — and every control that would read or write the page is replaced by one line asking the user to click the toolbar icon, whose click re-grants `activeTab` for that tab, opens the popup, and re-scans, after which the panel's controls return because the panel never re-arms itself on its own. Nothing silently fails and nothing silently re-grants.
...
Decision 2 shipped one context-menu entry (`ajh-answer-selection`), scoped to `contexts: ['selection']`, so a plain right-click with nothing selected showed nothing from this extension. A second entry now ships alongside it: `ajh-answer-open-panel`, registered on `contexts: ['page']` (fires on a plain right-click on the page body, no selection required), titled "Open answer panel" to match the popup's own control of the same name, and its click handler calls `openAnswerPanel` directly with no row added — there is nothing to prefill. `'page'` and `'selection'` are independent Chrome context categories rather than mutually exclusive ones, so right-clicking ON selected text now matches both. Verified against Chrome's documentation (developer.chrome.com/docs/extensions/reference/api/contextMenus, read 2026-09-05): "if more than one [item] from your extension is visible at once, Google Chrome automatically collapses them into a single parent menu" — so this shows as one "AI Job Hunter" submenu holding both entries in that case, not two flat top-level ones, and that folding is treated as expected, not a defect to route around with an explicit exclusion. No manifest/permission change: `contextMenus` was already declared for decision 2's entry.
