...
- **HTTP hardening:** Routes through `scraping::http::fetch_text` (not bare `shared().get()`) for timeouts + the shared byte cap (`MAX_BYTES`) + per-host rate limiter. Added opt-in `FetchOptions.timeout: Option<Duration>` field (default `None` for backward compat); the GitHub call passes `REQUEST_TIMEOUT` from `profile_import/github.rs`. See `scraping/http/mod.rs`.
...
- **i18n:** one key group, `build.extras.projects.github.*`, in both en and de (counts pluralized as `_one`/`_other`). The key roster is that group in `packages/translations`; en/de parity is what matters here, not a copy of the names.
...
- **Resume builder step:** `StepExtras` is one of the wizard steps using `useFieldArray` for repeatable sections (roster: `features/resume-builder/components/wizard-steps/`).
