...
- **Personal data**: any `Read` row that returns user data reaches the client's AI provider and its persisted transcript — the `commands` tool and `POLICY` in `policy.rs` are the list. Third-party posting text is fenced per surface — the curated tier's fence functions live as `fence_*` items in `agent_read.rs`/`agent_read/found_jobs.rs`, the generic tier's structural walk is `fence_scraped_fields` in `agent_call.rs`, and its bare-string replies (e.g. `ai_research_answer`, the most injection-prone reply on this surface) go through `reshape::fence_scalar_reply` instead — and the server's `instructions` say every span is untrusted. Never restate the covered field/command list here; it drifts (see each fn's own doc for the current one).
