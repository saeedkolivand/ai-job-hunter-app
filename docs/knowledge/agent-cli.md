...
- **Windows (Microsoft Store, MSIX)** — the `windows.appExecutionAlias` extension in `apps/desktop/src-tauri/windows/msix/AppxManifest.xml`. A packaged app cannot edit `PATH`, so the alias is what makes the same command name resolve. It is also what the pointer file publishes there: `current_exe()` inside a package points into a per-version directory a normal user cannot execute from, so `platform::msix::alias_exe_path` supplies the alias instead and `platform::config::agent_cli_exe_path` prefers it — the discovery contract below is therefore identical on both Windows flavours.
