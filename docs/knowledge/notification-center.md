...
- UI: `apps/tauri/src/renderer/components/layout/Titlebar/NotificationBell/index.tsx` — bell icon + unread badge + dropdown inbox; `apps/tauri/src/renderer/routes/__root.tsx` — `NotificationToastBridge` for toast rendering; app routes (`/autopilot`, `/autopilot/apply`, `/applications`) consume `focus` / `highlight` query params
