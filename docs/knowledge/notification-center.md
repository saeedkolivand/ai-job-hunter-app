...
- UI: `apps/desktop/src/renderer/components/layout/Titlebar/NotificationBell/index.tsx` — bell icon + unread badge + dropdown inbox; `apps/desktop/src/renderer/routes/__root.tsx` — `NotificationToastBridge` for toast rendering; app routes (`/autopilot`, `/autopilot/apply`, `/applications`) consume `focus` / `highlight` query params
- Sources: autopilot `apps/desktop/src-tauri/src/autopilot/mod.rs` (`tray::on_new_jobs`, pushes with `OsBanner::Always`); browser extension `apps/desktop/src-tauri/src/extension_bridge/mod.rs` (success branch, pushes with `OsBanner::WhenUnfocused`)
