...
The **quality pipeline** is the sole production path for apply-flow generation. It is a deterministic state machine whose stage list and order are pinned by [`QUALITY_STAGES`](../../apps/desktop/src-tauri/src/pipeline/resume/mod.rs); read the const rather than a copy, since a stage added there silently invalidates any list written here. See the [`pipeline/resume/mod.rs`](../../apps/desktop/src-tauri/src/pipeline/resume/mod.rs) module doc for the per-stage call table (indicating which are base costs, optional, or bounded), the [`PipelineStage`](../../packages/shared/src/events/pipeline.ts) event vocabulary, and [`PipelineRunDetail`](../../packages/shared/src/ipc/index.ts) for the persisted run shape.
