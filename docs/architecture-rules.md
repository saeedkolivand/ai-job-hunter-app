...
| **R15** | No `.display()` inside a `log::*!`/`tracing::*!` call (leaks an absolute, username-bearing path into logs/diagnostics bundles) — a `.display()` OUTSIDE a log/tracing macro, e.g. a user-facing `AppError` about the user's own file, is a different, legitimate case and is not scanned | arch test                              | none (clean)                                                                                                                                                                                                                                                                                                                                                                                                                              |
