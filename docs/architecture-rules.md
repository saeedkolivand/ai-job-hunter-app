...
### L0 — Shared infrastructure (`error`, `observability`, `performance`, `db`, `data_store`, `net`, `platform`, `browser`, `limits`, `vector`, `prompt_fence`)

- **Allowed deps:** other L0 modules only.
- **Forbidden deps:** L1, L2, L3. **No** `crate::commands`, `crate::scraping`, etc.
- **`prompt_fence`** (ADR-010 injection-fencing primitives — `fenced`/
  `FENCE_TAG_PATTERNS`/`neutralize_transcript_boundaries`/`JOB_CAP`/
  `RESUME_CAP`) relocated out of the L3 `agent` module in PR-5 step 1 so
  `pipeline`, `commands::ai_provider::structured`, and `extension_bridge` can
  all reach them without an upward import — `agent` itself was deleted
  entirely in PR-5 step 2. Genuinely dependency-free, no exceptions: `RESUME_CAP`
  used to re-export the GENERATED `ipc_contracts::agent_caps::AGENT_RESUME_TEXT_CAP`
  so the renderer's `improve_resume` entry point read the identical number —
  that entry point was deleted in PR-5 step 3, so the generator target and
  the `("prompt_fence", "ipc_contracts")` R7 exception it required were both
  removed in PR-5's review-fix round, and `RESUME_CAP` is a local literal now,
  same shape `JOB_CAP` already was.
...
- **Forbidden deps:** L3 (`crate::commands`, etc.). **No `#[tauri::command]`** (handlers
  ...
  domain stores + `net::http`). **Exception:** allowlisted application or domain store `mod.rs`
  files (e.g., `pipeline/cache`, `pipeline/runs`) may use `rusqlite::` directly; must use
  `db::open()` and implement `DataStore`.
...
- **Documented exceptions:** `pipeline`, `documents`-adjacent flows, and `cover_letter`
  ...
  `pipeline`/`documents`/`postings` use `crate::commands::ai_provider` for
  embeddings/provider routing. All allowlisted with `TODO(arch)` (target: inject an
  emitter port + relocate `ai_provider`). `autopilot_helpers` reads the L0
  `prompt_fence` primitives (`fenced`/`JOB_CAP`/`RESUME_CAP`) directly and owns its
  own `AUTOPILOT_NOTE_SYSTEM` prompt for its headless AI-notes step — read-only
  string construction, no controller invoked; it resolves its `Completer`/`Limiter`
  from the L3 caller instead of reaching into `crate::commands` itself. (The
  `autopilot_helpers -> agent` exception this used to require is gone: PR-5 step 1
...
| **R3**     | `rusqlite::` only in `db.rs`, `error.rs`, and per-domain store `mod.rs`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | arch test                              | infrastructure (db, error); L1 domain stores; L2 application stores (pipeline/cache, pipeline/runs); external read (scraping/board_login/import). **Authoritative allowlist:** `R3_ALLOW` in tests/architecture.rs                                                                                                       |
...
| **R7**     | No upward layer imports (L0→L1+, L1→L2+, L2→L3) via `crate::<mod>`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | arch test                              | `error→extraction` (W-9); `{pipeline,documents,postings,autopilot_scheduler}→commands` (W-1); `autopilot_helpers→events`, `platform→events` (accent watcher emits `SYSTEM_ACCENT_CHANGED` via the L3 events helper), `{pipeline,ai_config,scraping}→ipc_contracts` (generated cross-language consts) — each `TODO(arch)` |
| **R8**     | Every `.rs` file under `src/` (test files included) stays within `HARD_CAP_LOC`. A file already over the cap is ratcheted in the R8 baseline: it may shrink or be deleted, never grow. A new over-cap file has no entry and fails, so a split is the only way a baseline is rewritten (`R8_BLESS=1`, and only after a split actually happened)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | arch test (hard)                       | `HARD_CAP_LOC` in tests/architecture.rs; the ratchet is `tests/r8_baseline.txt` (`<loc>\t<rel>`, one line per over-cap file)                                                                                                                                                                                             |
| **R8b**    | Tests live out of line in Rust's standard layout: the tests of `<stem>.rs` go in `<stem>/tests.rs`, declared with a plain `#[cfg(test)] mod tests;` (for `dir/mod.rs`, `dir/tests.rs`); a large suite becomes `<stem>/tests/<topic>.rs`. Neither an inline `#[cfg(test)] mod … { … }` body nor `#[path]` wiring. Split by responsibility (sub-modules), never arbitrary chunks                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | arch test                              | `tests/r8b_inline_tests_baseline.txt` — a listed file may keep its inline body or `#[path]` wiring until it is split; a listed file with neither left fails (remove it)                                                                                                                                                  |
...
| **R15**    | No `.display()` inside a log/tracing macro call (leaks an absolute, username-bearing path into logs/diagnostics bundles). Matches both the qualified form (`log::warn!(`) and the bare one (`warn!(` after `use tracing::warn;`) — a `.display()` OUTSIDE a log/tracing macro, e.g. a user-facing `AppError` about the user's own file, is a different, legitimate case and is not scanned                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | arch test                              | none (clean)                                                                                                                                                                                                                                                                                                             |
| **EGRESS** | Every outbound `scheme://host` literal in `src/**` (plus `tauri.conf.json`/`vite.config.mts`/`apps/extension/src/manifest.ts`) is declared in the `EGRESS` inventory (ADR-0005), and every declared host is still present in source; dynamic-host sites (`https://{host}`-shaped literals) and CSP/manifest bare-scheme-or-wildcard sources are checked too. **Not covered** (documented in the file header): a redirect hop past `net::http.rs`'s first-hop guard (blocks only private-IP literals, not hostname rebinding, over up to 10 hops); a _new_ scheme-less host const (invisible to the extractor until a human adds it to both `EGRESS` and `SCHEMELESS`); a runtime-configurable `format!("{base}/…")` host (env override, user-typed IMAP host, user-pasted URL); a real host nested inside a `#[cfg(test)]` mod the stripper fails to detect | arch test (`tests/egress.rs`)          | `SCHEMELESS`-flagged bare-literal hosts (Personio, Rippling, Greenhouse discovery, IMAP) — **Authoritative inventory:** `EGRESS`/`SCHEMELESS`/`DYNAMIC_SITES` in `tests/egress.rs`                                                                                                                                       |
