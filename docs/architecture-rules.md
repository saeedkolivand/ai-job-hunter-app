...
- **Documented exceptions:** `pipeline`, `documents`-adjacent flows, and `cover_letter`
  ...
  emitter port + relocate `ai_provider`). `autopilot_helpers` reuses the L3 `agent`
  module's pure prompt-safety primitives (`fenced`/`JOB_CAP`/`RESUME_CAP`) + the
  shared `AUTOPILOT_NOTE_SYSTEM` prompt for its headless AI-notes step — read-only
...
| **R7**  | No upward layer imports (L0→L1+, L1→L2+, L2→L3) via `crate::<mod>`                                                                                                                                                                                                                                                                                                                         | arch test                              | `error→extraction` (W-9); `{pipeline,documents,postings,autopilot_scheduler}→commands` (W-1); `autopilot_helpers→events`, `autopilot_helpers→agent` (reuses `agent`'s pure prompt-safety primitives + shared system prompt for headless AI notes), `platform→events` (accent watcher emits `SYSTEM_ACCENT_CHANGED` via the L3 events helper), and `scraping→ipc_contracts` (generated credential-slot/codegen consts) — each `TODO(arch)` |
