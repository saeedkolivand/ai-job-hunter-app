...
| **R8b**    | Tests live in a sibling file, not in an inline `#[cfg(test)] mod … { … }` body: wire the module instead — `#[cfg(test)] #[path = "<stem>_tests.rs"] mod tests;` (for a `mod.rs`, a sibling `tests.rs` resolves without the `#[path]`). Split by responsibility (sub-modules), never arbitrary chunks                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | arch test                              | `tests/r8b_inline_tests_baseline.txt` — a listed file may keep its inline body until it is split; a listed file with no inline body left fails (remove it)                                                                                                                                                               |
