...
| **R7**  | No upward layer imports (L0→L1+, L1→L2+, L2→L3) via `crate::<mod>`                                                                          | arch test                              | `error→extraction` (W-9); `{pipeline,documents,postings,autopilot_scheduler}→commands` (W-1); `autopilot_helpers→events`, `platform→events` (accent watcher emits `SYSTEM_ACCENT_CHANGED` via the L3 events helper), and `scraping→ipc_contracts` (generated credential-slot/codegen consts) — each `TODO(arch)` |
