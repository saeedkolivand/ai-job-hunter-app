...
| anti-abuse rate + concurrency        | `limits` + `scraping/engine` | apply `RateLimited` error on `ai_generate`/`scrape_*` commands via the limiter; per-provider daily ceiling + concurrent-op bounds. Multi-board `scrape_boards` enforces a registry-derived server-side cap (bounded to the number of registered scrapers in `super::boards::all()`) in the engine; prevents request amplification even when payloads are crafted (CWE-770 defense), and scales transparently with board additions. |
