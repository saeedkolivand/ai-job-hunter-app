...
| `import { Store } from "@tauri-apps/plugin-store"` in features  | `useWindowControls()` or a dedicated service hook; store keys must be module constants, never user input (see `renderer/lib/onboarding-mirror.ts`) |
