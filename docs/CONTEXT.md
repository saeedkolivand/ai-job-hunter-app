...
subject line, recipient block) own the semantics. A layout owns arrangement only. Decorative elements (band in Banded, rail in Sidebar, initials device in Monogram) drop under ATS mode while core semantics remain.
...
**Help entry**:
One shipped question/answer pair of the help corpus — the `support.faq.*` translation keys,
assembled by `getSupportSections` (`apps/desktop/src/renderer/features/support/support-data.ts`).
The searchable Help & Support page lists them and the help chat answers from the ones retrieved
for a question; what the assistant may answer from is defined by the prompt builders in
[ADR-043](knowledge/decision-records/adr-043-help-chat-over-the-shipped-corpus.md), not here.
_Avoid_: "FAQ item", "doc", "article" (each implies prose the app does not have)

**Data glance**:
The read-only snapshot of the user's own counts and names the help assistant receives alongside
the entries. A prompt input, **not a data surface**: nothing is read back from it. Shape and limits:
`buildHelpDataGlance` (`packages/prompts/src/generate/help-chat/`).
...
name **where** a feature would live. Declared once as `SIDEBAR_NAV`
(`apps/desktop/src/renderer/components/layout/Sidebar/nav.ts`) and read by both the sidebar and the
help chat. Naming a page is not a claim about what is inside it.
...
**Document attach**:
The paired extension exporting a saved résumé or cover letter (`document.export`, rendered on
the user's own device) and placing it into the page the user is on — a `type=file` upload field
for a résumé, or a text field the user picks for a cover letter. Rides the same **Autofill**
opt-in as the rest of the "user's own data to the browser" consent class. The extension verifies
the file actually landed (re-reads the field) rather than assuming, since some custom
drag-and-drop widgets ignore a programmatic assignment.
...
container or its manifest rather than by app code. Two flavours: **Microsoft Store (MSIX)** on Windows,
and **Snap** on Linux, each with its own detection function (`platform::msix::is_packaged()`,
`platform::snap::is_packaged()`). Summarized by the enum `PackageFlavour`
(see `apps/desktop/src-tauri/src/platform/mod.rs`) with the aggregator `platform::is_packaged_build()` covering both. The closed
...
`docs/DEPLOYMENT.md` § Snap Store / § Microsoft Store (MSIX) is the operational view of each.
_Avoid_: "Store version" (the version number is identical across flavours); "sandboxed build" (Snap
is sandboxed; MSIX has write virtualization deliberately disabled)
