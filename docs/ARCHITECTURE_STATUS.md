...
Last updated: 2026-09-02 (help chat over the shipped corpus, ADR-043; previously searchable help page, ADR-041; previously PR #966: opt-in two-phase Autopilot ranking — see the "Ranking via keyword-coverage" row under Autopilot)
...
| Aggregator paging & spend budget   | ✅      | Adzuna/JSearch page to fill the request instead of one call per search; `SearchBudget` (`boards/aggregator/mod.rs`) splits the OUTPUT cap from the UPSTREAM spend target, so only manual searches buy the paid Apify tier. Metered fetches run `retries: 0`. PR #896 — see `docs/knowledge/scraping-domain.md` |
...
| Ranking via keyword-coverage | ✅     | Phase 1: unified on `documents::keywords::coverage_score` (default keyword-only path). Phase 2 (when semantic scoring enabled): top-`SEMANTIC_RERANK_MAX` re-rank via `match_resume::score_one` with per-job keyword-only degrade; label flips per job. ADR-020 + addendum. |
...
| Support route             | ✅     | Searchable help + grounded assistant (ADR-041/ADR-043); query the same entries with hybrid retrieval (semantic ranking opt-in)                                                                  |
...
| Apply-by-email draft                | ✅     | Subject + body persisted on the generation aggregate (`email_subject`/`email_body`), so a tab switch no longer discards the draft (PR #894). Keyed on the normalized `jobUrl` — a URL-less Application cannot be saved onto yet (recorded follow-up)                                                              |
...
| Pipeline strip on `/applications`   | ✅     | Stage cards above the list (`features/applications/lib/pipeline.ts` — `PIPELINE_GROUPS` is the source, pinned against `APPLICATION_STAGES` by `pipeline.test.ts`). Unfiltered → stage sections; any active filter → one flat list. PR #897, supersedes an ADR-0001 consequence                                    |
| Unified primary contact             | ✅     | `contact_name`/`contact_email` are canonical; `recipient_*` are deprecated aliases folded by the `unify_application_contact` migration (in place) and by `applications/contact.rs` (imported bundles) — the two must stay in lockstep. PR #897                                                                    |
| Follow-up reminders                 | ✅     | `next_action_at` sweeps to a notification through the notification center (`reminder_scheduler.rs` L2 → `commands::notifications::push_and_notify`); data half in `applications/reminders.rs`. Once per due date via a `next_action_notified_at` marker cleared on reschedule; terminal stages never nag. PR #897 |
| Status notes / interaction log      | ✅     | Optional note on a stage change, appended to the append-only `status_events` history. Server-bounded in BYTES by `MAX_STATUS_NOTE_BYTES` (`commands/applications.rs`); the renderer `maxLength` is a UX guard only. PR #897                                                                                       |
...
| Authored pages as routes            | ✅     | 7 routes: home, creature, how-it-works, privacy, download (faithful port of legacy static site), `/world` scroll-world (ADR-0019), `/accessibility` conformance statement (PR #924) — all `src/app/`                                                                                                                                                                                             |
...
| Scroll-world video route (`/world`) | ✅     | Scroll-scrubbed papercraft camera flight; vendored vanilla-JS engine (scrub-engine.js) + pre-produced video assets; desktop 16:9 + mobile 9:16 variants; a11y prefers-reduced-motion fallback; ADR-0019                                                                                                                                                                                          |
| Accessibility conformance statement | 🚧     | Landing site + desktop app **partially conformant** with WCAG 2.2 AA; extension not assessed. Audited violations all fixed in PR #924 — landing 0 across 9 pages, desktop home view 0 in both colour schemes. Open: the sub-AA `text-foreground/NN` ramp outside the home view (~460 usages), no landing a11y gate, extension unscanned, PDFs not PDF/UA-1 tagged. See `/accessibility`. PR #924 |
