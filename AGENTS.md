...
17. **Docs under `docs/` are thin pointers, never copies.** Describe shape and contracts and point at the owning source symbol; never paste drift-prone literals — weights, thresholds, counts, signatures, enum members, file lists. The test before you save: for every number or name on the page, ask _"if someone changed this in the code tomorrow, would this page silently become a lie?"_ If yes, replace it with a pointer to where it lives. This is the most frequently-raised review finding on this repo. **Generated pages are never hand-edited** — a file opening with a `GENERATED FILE` banner (`docs/API.md`) is rendered by a script, so fix the TSDoc on the contract member and re-run its generator (`pnpm gen:api`); editing the page fails `pnpm gen:api:check` in CI.
18. **Small Rust files, tests out of line.** Every `.rs` file under `apps/desktop/src-tauri/src` (tests included) stays under the `HARD_CAP_LOC` in `tests/architecture.rs` (R8); a file that genuinely reads worse split may go up to `R8_EXCEPTION_CEILING` if listed in `R8_EXCEPTIONS` with its reason. Split by responsibility (sub-modules), never arbitrary chunks — and shrink while you split: a split that only spreads the same lines over more files is incomplete; deduplicate (DRY), simplify (KISS), one responsibility per module/fn (SOLID), delete dead and speculative code (YAGNI), and trim restated-code comments, with behaviour and test assertions unchanged. Tests use Rust's standard out-of-line layout, never inline `mod tests { … }` and never `#[path]`: the tests of `foo.rs` live in `foo/tests.rs`, declared with `#[cfg(test)] mod tests;` (for `dir/mod.rs`, `dir/tests.rs`); a large suite becomes `foo/tests/<topic>.rs` (R8b). Existing offenders are ratcheted in `tests/r8_baseline.txt` / `tests/r8b_inline_tests_baseline.txt`: they may shrink, never grow; regenerate with `R8_BLESS=1 cargo test --test architecture` only after a split.
