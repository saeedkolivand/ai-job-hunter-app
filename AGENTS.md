...
Cursor and Windsurf read this file natively; Copilot does too, but not in every feature. The thin `.github/copilot-instructions.md` pointer documents this limitation. Claude Code reads it through the `@AGENTS.md` import at the top of `CLAUDE.md`, which adds Claude-only orchestration (subagent fleet, review gates, model tiering) on top. Cline reads it via the pointer in `.clinerules`.
...
Never output absolute paths, usernames, home dirs, drive letters, or temp/IDE paths in logs, PRs, commits, docs, screenshots, or stack traces. Always use repo-relative paths (`apps/desktop/src-tauri/src/main.rs`). The same rule applies to Git Bash-style paths (`/c/Users/…`).
...
11. **Remote data via React Query service hooks.** Do not use `useState + useEffect` fetching.
...
15. **Never bypass ESLint.** Do not use `// eslint-disable` or `@ts-ignore`. Add a scoped override in `eslint.config.mjs` with a reason. CI runs `lint:strict --max-warnings 0`.
