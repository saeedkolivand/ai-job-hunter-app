...
18. **Small Rust files, tests beside them.** Every `.rs` file under `apps/desktop/src-tauri/src` (tests included) stays under the `HARD_CAP_LOC` in `tests/architecture.rs` (R8). Split by responsibility (sub-modules), never arbitrary chunks — and shrink while you split: a split that only spreads the same lines over more files is incomplete; deduplicate (DRY), simplify (KISS), one responsibility per module/fn (SOLID), delete dead and speculative code (YAGNI), and trim restated-code comments, with behaviour and test assertions unchanged. Tests go in a sibling file, not an inline `mod tests { … }`: `foo.rs` → `foo_tests.rs` via `#[cfg(test)] #[path = "foo_tests.rs"] mod tests;` (for `mod.rs`, a sibling `tests.rs`) (R8b). Existing offenders are ratcheted in `tests/r8_baseline.txt` / `tests/r8b_inline_tests_baseline.txt`: they may shrink, never grow; regenerate with `R8_BLESS=1 cargo test --test architecture` only after a split.
