...
**Answer tools (opt-in AI drafting):** once the separate _AI answer drafting_ opt-in is on (**Settings → Accounts → Browser extension**), the popup's Answer tools section drafts and rewrites answers to an application's questions. Because a copy-only tool used to disappear the moment its draft was pasted (the popup closes on blur), the same tools also open in a persistent side panel: click **Open AI Job Hunter answer tool** in the popup (Chrome opens the panel, Firefox opens the sidebar — one control, both browsers), or right-click selected text on the page and choose **Answer this with AI Job Hunter**. See `docs/knowledge/extension-domain.md` for the shared state and gesture model behind the two views.
...
| `sidePanel` (Chrome only)                                  | Opens the Answer-tools panel, an extension page with **no** page access of its own, so a copy-only tool survives the click that uses it. Firefox needs no permission for this — it declares the `sidebar_action` manifest key instead.                                                                             |
