...
| `contextMenus`                                             | Adds two entries: “Answer this with AI Job Hunter” on selected text, and “Open answer panel” on a plain right-click (page background or an editable field). Either click is the user gesture that grants `activeTab` for that tab and opens the answer panel — no standing page access comes with it. |
