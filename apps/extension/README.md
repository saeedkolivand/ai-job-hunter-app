...
| `contextMenus`                                             | Adds the single “Answer this with AI Job Hunter” entry on selected text. Clicking it is the user gesture that grants `activeTab` for that tab and opens the answer panel with the selection as a question — no standing page access comes with it. |
