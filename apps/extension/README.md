...
<p align="center">
  ...
  <a href="https://addons.mozilla.org/en-US/firefox/addon/ai-job-hunter/"><img alt="Firefox published" src="https://img.shields.io/badge/Firefox-published-2ea44f?logo=firefoxbrowser&logoColor=white"></a>
</p>

This is the browser half of the **AI Job Hunter** job-import feature. An MV3 extension available for **Chrome on the Web Store** ([install](https://chromewebstore.google.com/detail/ai-job-hunter-%E2%80%94-job-impor/oaoekkgkhmgdfnpmfkpphgiikliaicll)) and **Firefox on AMO** ([install](https://addons.mozilla.org/en-US/firefox/addon/ai-job-hunter/)). It captures the job posting on the current tab and sends it to the **desktop app** running on your machine, over a private loopback WebSocket. No account. No remote backend. It is inert unless the desktop app is running and you have paired it.
...
**Assisted autofill (opt-in, off by default):** if you enable _Assisted form autofill_ in the desktop app (**Settings → Browser extension**), the popup also shows a **Fill this form** button. The first Fill on a given site shows a one-time confirmation naming what it will fill; tick "Don't ask again" to skip it there next time (undo any site from the extension's Settings → Sites). Clicking Fill fetches your saved **Contact Profile** (name, email, phone, location, LinkedIn/GitHub/website) **fresh from the desktop over the same loopback connection** and fills matching **empty** form fields on the current page, then shows an in-page summary of exactly what it filled. It **never auto-submits** — you review and submit yourself. It only fills unambiguous, visible, empty fields (email/name/phone/socials/location); it skips passwords, hidden fields, textareas (cover letters), search boxes, and ambiguous fields (referrer/emergency/confirm/company/…). A single "full name" split into first/last is flagged as a guess. The profile is used only for that one fill and is **never stored in the browser**; autofill therefore only works while the desktop app is running.

**Documents — attach & paste (same Assisted autofill opt-in):** the workspace panel's **Documents** tab exports your résumé or a saved cover letter — rendered on your own machine, in the format/template you pick — and either **attaches** it straight into the page's résumé upload field or **pastes**/**copies** the cover-letter text into a field you pick. Attaching verifies the browser actually accepted the file (re-reads the field after assigning it) rather than assuming; some custom drag-and-drop upload widgets ignore a programmatic assignment, and the panel reports that rather than claiming success. When a job has no saved documents yet, the tab offers **Generate in the app** / **Open in app** links that jump straight to that job in the desktop app. It never auto-submits, never stores the file in the browser, and never sends it anywhere except into the page you're on. If Fill matches nothing on a page, the Job tab falls back to showing your profile fields with a **Copy** button next to each — nothing is stored, you still paste it yourself.

**Answer tools (opt-in AI drafting):** once the separate _AI answer drafting_ opt-in is on (**Settings → Browser extension**), open the persistent side panel — the extension's **workspace** — and its **Answers** tab drafts and rewrites answers to an application's questions. The popup itself is a compact **launcher** (page context, Import/Check fit/Fill, "Open the panel →") and no longer hosts the Answer tools, because a copy-only tool used to disappear the moment its draft was pasted (the popup closes on blur). Open the panel from the popup's own control (Chrome opens the panel, Firefox opens the sidebar — one control, both browsers), or right-click selected text on the page and choose **Answer this with AI Job Hunter**. See `docs/knowledge/extension-domain.md` for the shared per-tab state and gesture model behind it.

**Settings:** an `options_ui` page, opened from the popup's "?" menu or the panel's gear icon — connection/pairing, the sites you've approved for Fill (with Forget), what the extension may do (switches for Assisted autofill / AI answer drafting / auto-track — toggleable from here, applying immediately in the app, with a desktop notification on every change so a flip is never silent), appearance (theme, default panel tab, on-page extras), shortcuts, and privacy/about.

**Reading app data into the panel (opt-in):** with Assisted autofill on, the panel can read a small, curated slice of your own app data (e.g. the current job's saved status, generated documents) over the same authenticated bridge, to show it in the workspace — never write it. This is gated the same way as autofill itself, capped in size, and refuses for anything the extension isn't allowed to see. See `docs/knowledge/extension-domain.md` for the exact gate.
...
**Known limit — assisted autofill fills the TOP-LEVEL document only.** The filler is injected without `allFrames`, so a form rendered inside an `<iframe>` (a Greenhouse/Lever/Ashby form embedded on a company careers page) is not filled — open the form on the ATS's own page and fill it there. Reaching those frames is not a flag flip: they are cross-origin, so it would require `host_permissions` for every ATS origin — far beyond the `activeTab`-scoped access this extension asks for (see "Permissions — minimal & justified") — plus cross-frame merging of the fill summary. Under-filling is the intended trade. The same limit applies to **Import**: a posting rendered inside a cross-origin `<iframe>` is unreachable from the top-level capture, so the parse would otherwise describe the wrapper careers page instead. When no description could be extracted AND the captured document embeds a third-party frame, the import is flagged partial and says so, rather than reporting a confident success under the wrapper page’s own heading.
...
| `contextMenus`                                             | Adds two entries: “Answer this with AI Job Hunter” on selected text, and “Open AI Job Hunter answer tool” on a plain right-click (page background or an editable field). Either click is the user gesture that grants `activeTab` for that tab and opens the answer panel — no standing page access comes with it. |
| `sidePanel` (Chrome only)                                  | Opens the workspace panel (Job/Answers tabs, more in later releases), an extension page with **no** page access of its own, so a copy-only tool survives the click that uses it. Firefox needs no permission for this — it declares the `sidebar_action` manifest key instead.                                     |
...
- **Panel read tier and Settings switches:** the same Autofill opt-in also lets the workspace panel read a small, curated set of the user's own app data (job/document state) over the loopback bridge to display it — read-only, capped in size, never persisted in the browser. The Settings page's switches (Assisted autofill / AI answer drafting / auto-track) can be flipped from the extension itself; each change is applied by the desktop app (the same code path as its own Settings screen) and surfaces a desktop notification, so it is never silent. Nothing here changes what leaves the device: still nothing, ever, except to the user's own paired desktop app.
...
- On open with the app **not running**: an empty body with a header **Retry** icon
  and a **"?"** menu (Help center / Settings / About); the status pill reads **"✕ App not running"**. No
  errors, no broken UI.
- With the app **running but unpaired**: a **pairing screen** asking for the
  64-character token from the app's Settings.
- With the app **running and paired**: the launcher view — page context once a job is detected, three actions (**Import this job**, **Check fit**, **Fill this form**) plus the "I already applied" checkbox, and a quiet **Open the panel →** into the workspace.
