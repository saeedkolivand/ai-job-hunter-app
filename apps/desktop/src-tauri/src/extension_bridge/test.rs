...
/// hand-picked `gate` cases: walks every ONE of the 165 real `POLICY` rows
...
/// dispatchable with no confirm. Walking all 165 real rows — not 2-3
...
fn agent_call_gate_matches_every_policy_rows_declared_effect() {
    ...
    // every one of the 165 rows must actually have been walked.
