...
fn tools(tier: Tier) -> Vec<Value> {
    let call_target_schema = |extra_properties: Value, extra_required: &[&str]| {
        let mut properties = json!({
            ...
            "input": { "type": "object", "description": "the command's input object (default {}), keyed by the target handler's own parameter names exactly as the app's UI sends them. Many write commands take ONE object parameter, so the payload must be nested under that parameter's name — e.g. {\"req\": {…}} or {\"prefs\": {…}}, not the bare object. An invoke_error whose detail names a missing key IS the recovery signal: re-send the same body wrapped under that key. On a command the `commands` tool marks as paged, `limit` and `cursor` belong to the paging layer, not to the command: they are read and stripped before dispatch." },
        });
