//! The loop itself — the three threads and the one channel the module doc's "Concurrency"
//! section describes: a reader that turns input into events, a single-flight dispatch worker,
//! and this thread, which classifies, answers and writes. Split from its own bookkeeping for the
//! same R8 LOC-cap reason the rest of this module tree was split.
