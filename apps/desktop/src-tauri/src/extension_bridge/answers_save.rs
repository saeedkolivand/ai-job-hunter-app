...
//! (read-only / an exact-match write to the user's OWN existing metadata) —
//! it rides the SAME assisted-autofill opt-in as `profile.get`/`fill`
//! (`BridgeState::autofill_enabled`): capture and fill are the two
//! directions of the one PII-adjacent consent gate.
//!
...
//! appends onto an existing pursuit.
//!
//! Split into [`consent`] (the `BridgeState` consent-lock helper) and
//! [`auto_flag`] (the `auto` field's validation + gate).
...
#[cfg(test)]
pub(in crate::extension_bridge) use self::auto_flag::is_auto_answers_save;
pub(in crate::extension_bridge) use self::auto_flag::{auto_flag_is_malformed, auto_save_refused};
use self::auto_flag::{MALFORMED_AUTO_FLAG_MESSAGE, SAVE_ANSWERS_ON_SUBMIT_OFF_MESSAGE};

/// Per-question / per-answer byte caps, char-boundary safe — untrusted
...
/// `answers.save` call may carry — a pathological page (or a hostile
/// collector) can't force an unbounded write; extras are silently dropped,
/// not rejected.
const MAX_ANSWERS_PER_CALL: usize = 50;

/// The `answers.save` outcome — see [`msg::ANSWERS_RESULT`] docs. `title`/
/// `company` ride the WIRE reply: the handler already loaded the Application
/// row for this verb, so surfacing them here is smaller than threading the
/// popup's separately-fetched `applied.check` state through.
#[derive(Debug)]
...
/// the caller derives `skipped` from it so an overflow past the cap counts
/// as skipped instead of vanishing silently. A malformed entry
/// (missing/non-string, or blank after trimming) is dropped, not rejected,
/// and never counts toward `raw_len`/`skipped` — it was never a well-formed
/// capture to begin with.
...
/// Core `answers.save`: gate on the autofill opt-in (see
/// [`super::AUTOFILL_OFF_MESSAGE`]), normalize + match `url` the SAME way
/// `resolve_applied_check`/`resolve_status_update` do, then merge the
/// (clamped, capped) captured answers onto the matched Application via
/// [`ApplicationStore::merge_answers`] — NEVER `merge_answers_by_question`,
/// which lets `incoming` win for a matching question; that's right for an
/// in-app rewrite but wrong here, where a stray re-capture must never
/// clobber an answer the user already reviewed. No match → a fixed sentinel
/// telling the user to import the job first; this verb never auto-creates.
...
pub(super) fn handle_answers_save(app: &AppHandle, req_id: &str, payload: &Value) -> String {
    ...
    let outcome =
        state.with_answers_save_consent_locked(|autofill_enabled, save_on_submit_enabled| {
            // An AUTO-flagged save (the submit-watch injected entry, not a user click) is
            // honored ONLY while the dedicated `saveAnswersOnSubmit` opt-in is on. A non-auto
            // save is unaffected, gated only by `autofill_enabled` below.
