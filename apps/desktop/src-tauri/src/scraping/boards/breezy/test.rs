...
// ---------------------------------------------------------------------------
// Slug guard — is_valid_breezy_slug (SSRF: subdomain DNS-label guard)
// ---------------------------------------------------------------------------

#[test]
fn slug_validation_accepts_valid_slugs() {
    assert!(is_valid_breezy_slug("acme"));
    assert!(is_valid_breezy_slug("my-company"));
    assert!(is_valid_breezy_slug("acme123"));
    assert!(is_valid_breezy_slug("a1b2-c3d4"));
    assert!(
        is_valid_breezy_slug(&"a".repeat(63)),
...
fn slug_validation_rejects_invalid_slugs() {
    assert!(
        !is_valid_breezy_slug("acme.corp"),
        "dot must alter URL authority — rejected"
    );
    assert!(!is_valid_breezy_slug("acme/corp"), "slash must be rejected");
    assert!(!is_valid_breezy_slug("acme@corp"), "@ must be rejected");
    assert!(
        !is_valid_breezy_slug("acme_corp"),
        "underscore must be rejected"
    );
    assert!(
        !is_valid_breezy_slug("-acme"),
    ...
    assert!(!is_valid_breezy_slug(""), "empty slug must be rejected");
    assert!(
        !is_valid_breezy_slug(&"a".repeat(64)),
