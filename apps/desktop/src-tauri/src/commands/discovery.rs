...
/// Every watched (starred) company, as full rows for the renderer. The store
/// ranks starred rows first, so an empty-query search filtered to `starred`
/// surfaces the whole watched set (bounded to the search cap — ample for a
/// personal tool). The autopilot resolver uses the lighter `store.watched()`
/// `(ats, slug)` pairs directly.
#[tauri::command]
pub fn discovery_watched(app: AppHandle) -> Value {
    let Some(store) = app.try_state::<crate::discovered::DiscoveredCompanyStore>() else {
        return json!([]);
    };
    let starred: Vec<_> = store
        .search("", 100)
        .into_iter()
        .filter(|c| c.starred)
        .collect();
