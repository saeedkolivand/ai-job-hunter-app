 ...
 * Rust pipeline's eight `pipeline:stage` names (PR-3 §6 of the staged-cutover
 * plan).
 *
 * Mirrors the posture of `resume-pipeline.machine`'s own `stageToEvent`: a
 * stage name this build doesn't know (a future stage) keeps the step where it
 * already is rather than regressing the checklist — forward-compat by
 * construction, not by a list that has to be kept in sync with the Rust stage
 * list by hand.
 */
