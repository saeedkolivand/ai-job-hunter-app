...
/**
 * Fetch a provider's live model catalogue; on failure, fall back to the last
 * successful list cached locally for that provider + base URL. Only rejects
 * when BOTH the live fetch fails AND no cache exists — the picker/settings
 * row can then tell "showing a cached list" apart from "nothing to show".
 */
...
export const useListProviderModels = (
  ...
  return useQuery({
    ...
    queryFn: () => fetchProviderModelsWithCache(api, provider, baseUrl),
    enabled: enabled && provider !== 'ollama',
    staleTime: QUERY_TIMES.VERY_LONG,
    // A rejection now means "no key / network error / bad response" AND no
    // cached list to fall back to — not transient, so retrying just re-pays
    // the backend's own timeout for no gain (see ModelSelector's
    // `modelQueries` for the matching rationale).
    retry: false,
  });
};
