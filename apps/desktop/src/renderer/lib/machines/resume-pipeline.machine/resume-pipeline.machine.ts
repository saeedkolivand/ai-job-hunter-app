 ...
 *
 * The full ladder belongs to MAX depth (Phase 4), where each section is its own
 * stage. At quality depth the draft is one pass, so only the two verdict states
 * are ever reached — `clean` and `needsChanges` — and they arrive together
 * after `validate`. The wider vocabulary lives here so Phase 4 fills states in
