  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('```')) {
      ...
      nodes.push(
        ...
        >
          {lang && (
            <span className="mb-1 block text-[10px] text-foreground/35 uppercase tracking-wider">
              {lang}
            </span>
          )}
          <code>{codeLines.join('\n')}</code>
      ...
      const sizeClass =
        level === 1
          ...
          : level === 2
            ...
            : 'text-sm font-medium';
      nodes.push(
        <div key={nodes.length} className={cn('mt-3 mb-1 text-foreground/90', sizeClass)}>
    ...
    if (line.startsWith('> ')) {
      ...
      nodes.push(
        <blockquote
          key={nodes.length}
          className="my-2 border-l-2 border-brand/40 pl-3 text-foreground/60 italic"
        >
    ...
    if (/^\d+\.\s/.test(line)) {
      ...
      nodes.push(
        <ol key={nodes.length} className="my-2 space-y-1 pl-4">
          {items.map((item, idx) => (
            <li key={idx} className="flex gap-2 text-sm text-foreground/80">
              <span className="shrink-0 tabular-nums text-brand/60 font-medium">
                {idx + startNum}.
              </span>
              <span>{renderInline(item)}</span>
            </li>
