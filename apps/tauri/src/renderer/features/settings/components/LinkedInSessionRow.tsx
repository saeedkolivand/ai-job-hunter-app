...
export function LinkedInSessionRow({ board }: LinkedInSessionRowProps) {
  ...
  const handleConnect = async () => {
    try {
      ...
      if (res?.error) {
        ...
        notify('LinkedIn sign-in was cancelled or timed out.', 'warning');
      }
    } catch (err) {
      notify(err instanceof Error ? err.message : 'LinkedIn connection failed.', 'error');
    }
  };
  ...
  const handleDisconnect = async () => {
    try {
      ...
      notify('Disconnected from LinkedIn.', 'success');
    } catch (err) {
      notify(err instanceof Error ? err.message : 'Failed to disconnect from LinkedIn.', 'error');
    }
  };
