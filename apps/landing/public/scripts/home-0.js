...
(function(){
  try{
    ...
    console.log('%c(the code’s Apache-2.0 — yours to read, fork & sell. unlike my prospects. a few vendored datasets stay non-commercial though — see the README’s licence note.)', soft);
  }catch(e){}
})();
...
(function(){
  ...
  (function(){
    ...
    var line2=document.getElementById('journey-path-2'), tip2=document.getElementById('journey-tip-2');
    var len=0, len2=0, built=false, tbl=null, tbl2=null, tipS=1;
    ...
    function build(){
      if(getComputedStyle(wrap).display==='none'){ built=false; return; }
      ...
      var clearEls=document.querySelectorAll('.dialog, .feat-grid'), clearB=[];
      ...
      var gateEls=document.querySelectorAll('.testi h2'), gateT=[];
      ...
      function route(startSide){
        var P=[[(startSide%2===0)?Lx:Rx, 0]], side=startSide, prevEy=0;
      ...
      line2.setAttribute('stroke-dasharray', len2);
    ...
    function makeTable(ln,L){
      var TL=[], TY=[], steps=Math.max(40, Math.round(L/24)), prevY=0;
