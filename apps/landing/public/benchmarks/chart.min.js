/*!
 * Vendored copy — apps/landing/public/benchmarks/chart.min.js
 * Source:  https://cdn.jsdelivr.net/npm/chart.js@2.9.4/dist/Chart.min.js
 * Version: chart.js 2.9.4 (2020) — last v2 release; matches the v2 options
 *          API this page's inline script already uses (xAxes/yAxes,
 *          tooltips, onClick._index), which chart.js v3+ does not accept
 *          without a rewrite. Bumped from 2.9.2 to pick up the fix for
 *          GHSA-h68q-55jf-x68w (CVE-2020-7746, prototype pollution in
 *          helpers.merge); 2.9.4 has zero OSV advisories.
 * License: MIT (Chart.js Contributors) — see banner immediately below.
 * Fetched from jsDelivr and verified byte-for-byte against jsDelivr's own
 * published package hash for this exact version+file (sha256 base64
 * t9UJPrESBeG2ojKTIcFLPGF7nHi2vEc7f5A2KpH/UBU=, https://data.jsdelivr.com/v1/
 * packages/npm/chart.js@2.9.4) before vendoring, 2026-08-17.
 *
 * Vendored instead of CDN-loaded or SRI-pinned: this origin also stores a
 * GitHub PAT in localStorage (mission-control, see config.ts::tokenKey) —
 * per CspMeta.tsx's ORIGIN INVARIANT, no third-party script may execute on
 * ANY page of this origin, ever, so no external host belongs in a <script src>
 * here even with an integrity hash. Update this file (re-fetch + re-verify
 * the hash) rather than pointing back at a CDN.
 */
/*!
 * Chart.js v2.9.4
