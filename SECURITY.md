...
- **Local-first:** Your jobs, résumés, generations, applications, and tracked job data live in a local database on your machine. There is **no app-operated backend** collecting them, and no behavioural analytics. The app sends data out only to services you configure or invoke: the AI provider you configure (which receives the résumé and job text you ask it to generate from), job-board scraping, an optional opt-in web search, an on-launch updater check (GitHub), user-typed location autocomplete (OpenStreetMap), opt-in company-logo enrichment (Clearbit, default **off**), and opt-in email-confirmation watching via IMAP (credential user-supplied and OS-keychain-backed; email content never leaves the device). The app's own outbound reporting is limited to **crash reports** (error + stack trace, OS, app version) sent to Sentry (default on, asked at first run, revocable in Settings → Privacy, redacted of paths/links/hosts/e-mails/credentials before transmission; user documents and job data are never included—see [ADR 0005](docs/adr/0005-network-egress-privacy-boundary.md)).
...
- **Untrusted external content is fenced:** Web-sourced company research is wrapped in an untrusted block before reaching any prompt (reference-only; the model is told to ignore embedded instructions). AI output is treated as untrusted by the renderer.
