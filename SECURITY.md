...
- **Local-first**: your jobs, résumés, generations, applications, and tracked job data live in a local database on your machine; there is **no app-operated backend** collecting them, and no behavioural analytics. The app sends data out only to services you configure or invoke: the AI provider you configure (which receives the résumé and job text you ask it to generate from), job-board scraping, an optional opt-in web search (the AI provider's own, or Exa when a key is stored), version checks (GitHub, 10 s after launch then every 4 h), user-typed location autocomplete (Photon), opt-in company-logo enrichment (Clearbit, default **off**), and opt-in email-confirmation watching via IMAP (credential user-supplied and OS-keychain-backed; email content never leaves the device). The app's own outbound reporting is limited to **crash reports** (error + stack trace, OS, app version) sent to Sentry, on by default, asked at first run, revocable in Settings → Privacy, and redacted of paths/links/hosts/e-mails/credentials before transmission; user documents and job data are never included (see [ADR 0005](docs/knowledge/decision-records/0005-network-egress-privacy-boundary.md)).
