...
Every entry in CANDIDATE PROJECT / PUBLICATION LINKS must have exactly one matching item somewhere in the output — drop none, and follow each entry's own instruction above for how to name it. Keep any [label](url) markdown links already present in <candidate_resume> intact on their items.
