...
export const APPLICATION_QUESTIONS: ApplicationQuestion[] = [
  ...
  },
  {
    ...
    category: 'logistics',
    guidance:
      'This field often accepts only a number. A number may be given ONLY when grounded in <applicant_details> or a <salary_context> market range, never invented from nothing. Precedence for the "Number:" line: (1) If a <salary_context> market range is present, ALWAYS include a number: if <applicant_details> states a salary expectation that contains an actual number and that expectation is in the SAME currency as <salary_context>, use it (its upper bound, if given as a range); if that figure falls below the market range, use the lower bound of that market range instead (never undersell against the market); if it is already at or above the market range, keep it as stated. If the stated expectation is in a DIFFERENT currency than <salary_context>, or its currency is ambiguous, do NOT convert or floor it against the market range: use the originally stated figure and currency for the Number line as given, and mention the market range separately in your prose as extra context only. If there is no numeric expectation at all, use the midpoint of the market range, rounded to the nearest whole number. Always state the market range itself in your prose too. (2) If there is NO <salary_context> market range, use a number only when <applicant_details> states a salary expectation that contains an actual number (its upper bound, if given as a range); when NEITHER a numeric expectation NOR a market range is present, or the stated expectation contains no number (e.g. "competitive", "negotiable", "DOE"), stay non-committal and omit that final line entirely; never invent a figure. Whichever figure applies, state it in your prose, then end with one line reading exactly "Number: <integer>" containing ONLY digits, no currency symbol and no thousands separator (e.g. "Number: 72000").',
  },
  {
    id: 'availability',
...
4. A salary figure may be stated ONLY when grounded in <applicant_details> or <salary_context> (never invented from nothing). When <salary_context> (a researched market range) is present, ALWAYS state a figure: when the expectation in <applicant_details> contains an actual number and is in the SAME currency as <salary_context>, state it without hedging (as a range only if given as a range), but if that figure falls below <salary_context>, state the lower bound of <salary_context> instead (never undersell against the market); if it is already at or above <salary_context>, keep it as stated. When the stated expectation is in a DIFFERENT currency than <salary_context>, or its currency is ambiguous, do NOT convert or floor it against the market range: state the figure and currency exactly as given in <applicant_details>, and mention the market range separately in your prose as context only. If no numeric expectation is stated, state the midpoint of <salary_context>. Mention the market range itself in your prose whenever <salary_context> is present. When <salary_context> is NOT present, a figure may be stated only when <applicant_details> lists a salary expectation that contains an actual number, and then state it without hedging (as a range only if given as a range); otherwise (no expectation, or a non-numeric expectation like "competitive"), answer non-committally ("open to discussing") and never state a number. NEVER fabricate a number or round beyond what is stated or researched. For other logistics (start date, notice period, remote/hybrid preference), use <applicant_details> when present; if a needed detail is absent, answer non-committally. NEVER invent a number or date. This matters: these answers may be submitted automatically.
...
export function buildApplicationAnswerPrompt(params: {
  ...
  guidance?: string;
  /** Web-researched market salary range (salary question only) — a validated,
   *  numbers-only reference; see {@link buildSalaryRangeBlock}. Absent when no
   *  lookup ran or it found nothing reliable. */
