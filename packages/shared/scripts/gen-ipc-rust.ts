...
import { EFFORT_TIMEOUT_MULTIPLIER, STREAM_BASELINE_SECS } from '../src/ai-timeouts.js';
import { EVENT_CHANNELS, PIPELINE_STAGE_PHASES } from '../src/events/index.js';
...
function genEvents(): string {
  ...
  // vocabulary only.
  const phases = PIPELINE_STAGE_PHASES.map((p) => JSON.stringify(p)).join(', ');
  // Match rustfmt: a const whose single-line form exceeds max_width (100) breaks
  // after `=` and indents the value 4 spaces, so the generated file stays
  // `cargo fmt --check`-clean as well as `gen:ipc:check`-stable.
  const phasesSingleLine = `pub const PIPELINE_STAGE_PHASES: &[&str] = &[${phases}];`;
  const phasesDecl =
    phasesSingleLine.length > 100
      ? `pub const PIPELINE_STAGE_PHASES: &[&str] =\n    &[${phases}];`
      : phasesSingleLine;
  return [
    '// @generated by `pnpm gen:ipc` — DO NOT EDIT BY HAND.',
...
function genDateFilters(): string {
  ...
  // after `=` and indents the value 4 spaces, so the generated file stays
  // `cargo fmt --check`-clean as well as `gen:ipc:check`-stable.
  const singleLine = `pub const DATE_FILTER_OPTIONS: &[&str] = &[${list}];`;
  const decl =
...
function genAiIntents(): string {
  const list = AI_GENERATE_INTENTS.map((t) => JSON.stringify(t)).join(', ');
  const singleLine = `pub const AI_GENERATE_INTENTS: &[&str] = &[${list}];`;
  const decl =
    singleLine.length > 100
      ? `pub const AI_GENERATE_INTENTS: &[&str] =\n    &[${list}];`
      : singleLine;
  return [
    '// @generated by `pnpm gen:ipc` — DO NOT EDIT BY HAND.',
    '// Source of truth: packages/shared/src/schemas/index.ts',
...
function genStreamTimeouts(): string {
  ...
  const entries = Object.entries(EFFORT_TIMEOUT_MULTIPLIER)
    .map(([tier, mult]) => `("${tier}", ${Number.isInteger(mult) ? `${mult}.0` : mult})`)
    .join(', ');
  // Match rustfmt: a const whose single-line form exceeds max_width (100) breaks
  // after `=` and indents the value 4 spaces, same as `genDateFilters`/`genAiIntents`.
  const singleLine = `pub const EFFORT_TIMEOUT_MULTIPLIER: &[(&str, f64)] = &[${entries}];`;
  const tableDecl =
    singleLine.length > 100
