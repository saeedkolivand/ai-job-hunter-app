...
**Review gates:** the Stop hook (`.claude/hooks/review-gate.mjs`) is **deterministic-only** (ast-grep Tier-0 + ledger re-emits, near-free, blocks on introduced arch violations and unresolved findings). **LLM review happens pre-PR via the agent internal chain + CodeRabbit** (the automatic `🤖 AI Review OK` CI gate was retired 2026-09-22 — `✅ CI OK` is the only required check); metrics in `.claude/.review-metrics.jsonl` include token usage + cost.
