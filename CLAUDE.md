...
- **Agents** (`.claude/agents/`) — 13 specialized reviewers/producers: `resume-export-expert`, `job-match-expert`, `scraping-applier-expert`, `ai-provider-expert`, `rust-backend-architect`, `tauri-security-reviewer`, `frontend-reviewer`, `performance-profiler`, `testing-reviewer`, `test-author`, `pdf-docx-generator`, `cleanup`, `project-steward`. `cleanup` always runs immediately before `project-steward` (dead-code audit; report-first, SAFE-tier deletes only after confirmation).
- **Commands** (`.claude/commands/`) — `/review-{rust,security,performance,ats,resume,template,export,frontend,scraping,ai}`, `/implement-feature`, `/fix-bug`, `/refactor-module`, `/add-tests`, `/analyze-job-ad`, `/improve-ats-score`, `/cleanup`, `/update-docs`, `/prepare-release`.
