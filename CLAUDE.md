...
- **Agents** (`.claude/agents/`) — 15 specialized reviewers/producers: `resume-export-expert`, `job-match-expert`, `scraping-applier-expert`, `ai-provider-expert`, `rust-backend-architect`, `tauri-security-reviewer`, `frontend-reviewer`, `performance-profiler`, `testing-reviewer`, `test-author`, `pdf-docx-generator`, `code-quality-reviewer`, `code-quality-author`, `cleanup`, `project-steward`. `cleanup` always runs immediately before `project-steward` (dead-code audit; report-first, SAFE-tier deletes only after confirmation). `code-quality-reviewer` (read-only audit) + `code-quality-author` (write/refactor) are a cross-cutting clean-code/DRY/KISS pair — **on-demand only** (like `cleanup`, not in the Stop-gate routing); not part of the mandatory per-change sequence.
...
- **Sonnet** — balanced implement/review: `frontend-reviewer`, `scraping-applier-expert`, `pdf-docx-generator`, `test-author`, `testing-reviewer`, `performance-profiler`, `code-quality-reviewer`, `code-quality-author`.
