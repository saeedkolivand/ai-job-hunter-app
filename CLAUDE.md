...
- **`ponytail`:** Lazy-senior-dev default (laziest solution that works; YAGNI, stdlib/native over deps). Intensity `full`; toggle with `stop ponytail` / `normal mode`. Source: `ponytail@ponytail` plugin.
- **`grill-with-docs`:** Before finalizing any non-trivial plan/design (including before `ExitPlanMode`), stress-test it against the domain model and ADRs. Skip for trivial/one-line/docs changes. Source: the user-level `grill-with-docs` skill.

### Agent system

Domain pairs: a write-capable **author** implements, an independent **critic** audits (authors never approve their own work). Full pipeline detail in `.claude/` (agents/skills/commands), routing in `.claude/review-routes.json`, knowledge base in `docs/knowledge/`. Drift guard: `pnpm check:agent-system` (pre-push + CI).
...
**Per-change defaults (cost-tiered via ADR-025):**

- **Trivial diffs** (docs/config/comments/renames/single-file ≤10 lines): the **main session edits directly** with no swarm. The deterministic Tier-0 Stop gate and the pre-push AI review still cover them.
- **Single-domain, non-risk changes:** author to **ONE sibling critic** (resolve HIGH/CRITICAL; LOW/MEDIUM advisory), then if testable logic, `test-author` to `testing-reviewer`.
- **Risk-bearing** (any `tauri-security-reviewer` secondary glob) **or multi-domain:** full trio including security critic (three critics max per task).
- `project-steward` closes **once per PR** (docs/lessons sync plus `graphify update .`), not once per change. Context flows via `.claude/scratch/<task>.md`. Orchestrate all sub-agents from the main session (agents cannot call agents).
- **Before a PR:** `/review-security` (HIGH/CRITICAL block) then `/review` (🔴+🟠 block). Both complement CodeRabbit.

**Review gates:** The Stop hook (`.claude/hooks/review-gate.mjs`) is **deterministic-only** (ast-grep Tier-0 plus ledger re-emits; near-free, blocks on introduced arch violations and unresolved findings). The **LLM review runs at pre-push** (`scripts/pre-push-review.mjs`, ADR-0008 ratchet) and in CI. Metrics in `.claude/.review-metrics.jsonl` now include token usage and cost.
...
- **Cross-session recall:** Agents may call `mcp__mcp-search` (claude-mem plugin; absent if not installed). Honor `docs/` path-privacy and `<private>` for PII.

**Model & effort tiering** (per-agent frontmatter; aliases track the current family): Opus for last-line critics. `pr-reviewer` + `tauri-security-reviewer` pin `effort: xhigh`; the other Opus critics (`rust-backend-architect`, `ai-provider-expert`, `job-match-expert`) run `high`. Sonnet for authors and balanced critics (default `high`). Haiku for `project-steward` and `finding-verifier` (`low` fine for mechanical runs). Escalate a spawn to Opus for genuinely hard work; escalate to Fable only for hard, ambiguous, cross-domain work. Never use these as frontmatter defaults, and never for security review.
