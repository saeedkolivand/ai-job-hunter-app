...
4. Spawn **only** the `ai-provider-expert` subagent (Agent) as Primary Owner. Enforce the **zero-change provider rule** (new provider = config + adapter only; no business-logic coupling = HIGH). Add `tauri-security-reviewer` (prompt injection / data leakage) as Secondary on risk.
