...
2. Spawn `job-match-expert` (Agent) — compute the gap: missing keywords/skills/requirements vs the target job; produce actionable, **truthful** suggestions (never fabricate experience).
3. Spawn `resume-export-expert` (Agent) — verify the proposed changes keep the document **ATS-safe** (parseable layout, section naming, no formatting that breaks extraction).
