...
Per-invocation `model: "sonnet"` overrides `pr-reviewer`'s `model: opus` frontmatter; the `effort: xhigh` pin does not get overridden — intentional, the ensemble varies model, not effort.
...
3. **Verify single-source findings**: spawn `finding-verifier` subagents in parallel, passing each finding JSON + its diff hunk — **at most 5 verifier spawns total** (each spawn costs a full fresh context): with ≤5 single-source findings it's one finding per verifier; with more, batch them evenly across the 5 verifiers (a verifier scores each finding in its batch independently). **Drop anything scoring < 80.** For survivors, the verifier's score **replaces** the finding's confidence (`score / 100`) — fresh-context verification is the strongest signal we have, so it, not the originating pass's self-rating, feeds the verdict. Do NOT re-verify findings yourself in-session (same-session self-review is the weakest form).
