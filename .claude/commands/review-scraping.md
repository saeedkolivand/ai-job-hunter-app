...
4. Spawn **only** the `scraping-applier-expert` subagent (Agent) as Primary Owner. Add `tauri-security-reviewer` (cookies/sessions/egress) or `performance-profiler` (concurrency) as Secondary only on risk — **≤3 reviewers**.
