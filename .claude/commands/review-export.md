...
4. Spawn **only** the `resume-export-expert` subagent (Agent) as Primary Owner (review). `pdf-docx-generator` owns rendering _implementation_; `tauri-security-reviewer` joins as Secondary only if export touches file/temp/egress paths.
