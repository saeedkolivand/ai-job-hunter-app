...
3. Target = `$ARGUMENTS` if given, else the current `git diff` under `apps/extension/`, `apps/desktop/src-tauri/src/extension_bridge/`, `commands/extension_bridge.rs`, `packages/shared/src/ipc/extension-protocol-constants.ts`.
4. Spawn **only** the `extension-reviewer` subagent (Agent) as Primary Owner. Add `tauri-security-reviewer` (auth/token/permission/data) as Secondary on risk — **≤3 reviewers**.
