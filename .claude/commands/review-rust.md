...
5. Spawn **only** the `rust-backend-architect` subagent (Agent) as Primary Owner over the target. Add `tauri-security-reviewer` and/or `performance-profiler` as Secondary **only** if the change is risk-bearing — **≤3 reviewers** total.
