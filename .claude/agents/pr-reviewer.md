...
- `pnpm typecheck` (tsc `--noEmit`, all packages) — TS errors are 🔴.
- `pnpm lint:strict` (`eslint --max-warnings 0`) — esp. `react-hooks/exhaustive-deps`. A lint error the diff introduced is at least 🟠.
- If `apps/desktop/src-tauri/**` changed: in that dir, `cargo fmt --all -- --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test` (+ `--test architecture` if layering/modules changed).
...
- If `packages/shared/**` IPC contracts/schemas changed: `pnpm gen:ipc:check` (must be clean) and confirm `mock-client.ts` mirrors any new method.
- **Targeted tests**: run the test files covering the touched code (not the full suite) — `pnpm --filter @ajh/desktop test <paths>` / the owning package filter.
