...
- **Cache/derived-state invalidation on mutation** — a write that changes backing data must invalidate **every** cache/derived value keyed on it, not just the row written. A `posting` text mutation must drop the cached **embedding** and any text-hash-keyed score (#486 left a stale snippet embedding so the resolve-on-open re-score reused the old text), and the renderer query that reads it must be invalidated too. Trace each dependent store/cache and confirm it's refreshed.
- **Bounded-means-bounded** — an external `Command`/IO call the code or docstring claims is "bounded"/"short timeout" must actually enforce one. A blocking `Command::output()` with no `wait_timeout` can hang (#486 `flatpak --version`/`info` probes claimed a timeout they didn't have); a missing-binary/slow path must not block the caller (e.g. startup/detection).
- **Env/process-mutation scoping + rollback** — a global `set_var`/`LD_PRELOAD`/process tweak must be **scoped to the case that needs it** (not applied to every run — #486 disabled WebKit DMABUF/compositing for _all_ Linux instead of just the broken AppImage+Wayland case) and **rolled back on failure paths** so the still-running process and its children don't inherit bogus state (a failed re-exec must restore the prior `LD_PRELOAD` and clear its guard env).
- **Parallel-table / registry consistency** — adding a value to one table (a detection list, the board/provider/applier registry, a manifest map, an enum) requires updating every parallel table that must stay in lockstep. #486 detected Vivaldi but never registered its native-messaging manifest. Grep the sibling tables for the new id.
...
- **Invalidation key must actually match the queries it targets** — `invalidateQueries({ queryKey })` defaults to **prefix/partial** match (it hits any query whose key _starts with_ the filter key); a filter key with a **trailing `undefined` or extra segment** (`keys.x.y()` → `['x','y',undefined]`) is compared element-by-element and will **not** match the typed queries (`['x','y','viewed']`). Invalidate the shorter prefix (`['x','y']`), or use `exact`/a `predicate` deliberately. #486 shipped this exact bug twice (`usePersistJob`, `useClearInteractions` → stale viewed/saved badges). Whenever the diff mutates data, confirm the key it invalidates actually matches the queries that render the affected UI.
- **Nullable at the IPC/contract boundary** — a value the shared contract/Zod types as optional/nullable (a detected browser `path`, an absent field, `null` from a command) must be guarded before string/array ops; `.toLowerCase()`/`[0]`/`.split()` on a possibly-missing value throws (#486 `getBrowserLabel`). Trust the contract's declared nullability over the happy path.
