...
| `packages/shared/**` (IPC contracts/schemas)                            | After contract edits, `pnpm gen:ipc:check` must be clean; mock-client must mirror the new method (tsc fails otherwise). No React/Node in `shared`.                                                                                                                                                                          |
...
- **`noUncheckedIndexedAccess` is strict**: tests pass under esbuild (`pnpm test`) but `tsc`/CI is stricter. Always trust `pnpm typecheck`, and guard array indices (no `!`).
...
- **Hypothetical-future-duplicate nits on short curated literals** (e.g. "EXPECTED_HREFS is an array not a Set, a duplicate would inflate the count") are below the actionable bar when the check logic itself is uncompromised and the literal is small enough to eyeball in review. (False positive by a /review sonnet pass on the landing-tsx PR1 branch; verifier scored 38.)
- **React 19 leaves plain `<link rel="stylesheet">` (no `precedence`) in the BODY at its render position — only `<meta>`/`<title>` hoist to head.** Before raising any "head/CSP ordering changed" finding on apps/landing, byte-check the actual built out/*.html for BOTH versions; the landing Fonts link has always rendered in body after mission-control's head CSP meta. (False positive by a /review sonnet pass on the landing-tsx PR1 branch — its quoted byte positions were misread; verifier scored 2.)
