...
**graphify MCP (optional, local-only):** graphify ships a stdio MCP server (`python -m graphify.serve graphify-out/graph.json`; there is **no `graphify --mcp` flag**). `.mcp.json` is gitignored here (it's the per-developer MCP config), so wiring stays local. To enable it: (1) `uv tool install --with mcp graphifyy` — the serve module needs the `mcp` package, absent by default; (2) add graphify to your own `.mcp.json`: `{ "mcpServers": { "graphify": { "command": "node", "args": ["scripts/graphify-mcp.mjs"] } } }`. The committed `scripts/graphify-mcp.mjs` launcher resolves the machine-specific interpreter from `graphify-out/.graphify_python` at runtime, so no home path is ever committed. It mostly benefits the **main session** — spawned sub-agents already reach the graph through Bash (`rtk graphify query …`), so the durable token lever for them stays the graphify-first prompt directive + pre-harvest above.
