...
- **Real controls** — a click handler on a `<div>`/`<span>` needs `role="button"` + `tabindex="0"` + an Enter/Space keydown handler (or just use `<button>`/`@ajh/ui`). Icon-only buttons need `aria-label` (not just `title`). Toggle/filter/segmented controls need `aria-pressed`.
