...
- **New IPC capability** — touches 5 files in order: `packages/shared/src/ipc/contracts.ts` → `apps/tauri/src-tauri/src/commands.rs` → `apps/tauri/src/tauri-client.ts` → a `renderer/services/` hook → `services/query-client.ts` query key.
