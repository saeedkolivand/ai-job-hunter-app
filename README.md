...
| Need to report a crash or bug                 | Open **Support → Contact/Feedback → Export Bundle** — saves `ajh-diagnostics-<date>.zip` (contains only `system-info.txt`, `crashes.log`, and `logs/` — all redacted; no résumés, API keys, job data, or database). Attach it to a new [GitHub issue](https://github.com/saeedkolivand/ai-job-hunter-app/issues/new). For security vulnerabilities use [SECURITY.md](SECURITY.md) instead — do not post them publicly. |
