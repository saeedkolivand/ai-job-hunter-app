...
Your résumés, generations, applications, tracked job data, and credentials never leave your device — there is no telemetry and no app-operated backend. Outbound network calls are limited to a few disclosed classes: the AI provider **you** configure, job-board scraping (the app's core function), an optional web search you explicitly enable, an on-launch updater check (GitHub), user-typed location autocomplete (OpenStreetMap), and opt-in company-logo enrichment (Clearbit, default **off**). None of them receive your personal data — see [ADR 0005](docs/adr/0005-network-egress-privacy-boundary.md).
