...
Your résumés, generations, applications, and tracked job data live in a local SQLite database on your device (credentials live in the OS keychain); there is **no app-operated backend** collecting them, and no behavioural analytics. The app sends data out only to services **you** configure or invoke: your AI provider (which receives the résumé and job text you ask it to generate a résumé or cover letter from), the job boards you scrape, an optional web search you explicitly enable (your provider's own, or Exa if you add a key), version checks (GitHub, 10 s after launch then every 4 h), user-typed location autocomplete (Photon), opt-in company-logo enrichment (Clearbit, default **off**), and opt-in email-confirmation watching via IMAP (credential user-supplied and OS-keychain-backed; email content never leaves the device). The one thing the app sends on its own behalf is a **crash report** when it breaks (error + stack trace, OS, app version) to Sentry, on by default, asked during setup, switchable off in Settings → Privacy, and redacted of paths, links, hosts, e-mails and credentials before sending. Your documents and job data are never included, and the browser extension is excluded entirely; see [ADR 0005](docs/knowledge/decision-records/0005-network-egress-privacy-boundary.md).
